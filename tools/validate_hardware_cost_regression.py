...
def main():
    ...
    check("source hashes recorded",all(all(len(x["sha256"])==64 and x["bytes"]>0 for x in s["source_files"]) for s in revisions),"sha256 and byte count")
    ...
    out=path(a.output); required=["paper_revision_table.csv","paper_category_table.csv","paper_delta_table.csv","paper_regression_report.md","paper_revision_overview.png","paper_category_power.png","regression_summary.json"]
