#!/usr/bin/env python3
"""Issue a hash-pinned authorization for B12's sole global-route attempt."""
...
PHYSICAL_AUTH = PHYSICAL / "b12_route_pin_smoke_execution_report.json"
...
OUTPUT = REPORT / "b12_global_route_authorization.json"
...
def main() -> int:
    if OUTPUT.exists():
        raise FileExistsError(f"refusing to overwrite B12 route authorization: {OUTPUT}")
    ...
    route_outputs = [
        PHYSICAL / "b12_global_route_invocation.json",
        PHYSICAL / "b12_global_route_execution_report.json",
        PHYSICAL / "b12_global_route.log",
        REPORT / "b12_residual_congestion_analysis.json",
        REPORT / "b12_residual_congestion_analysis.md",
    ]
    artifact_root = Path("/dev/shm/wbq_b12_phase6_10/quad_local_b12")
    route_outputs.extend(
        artifact_root / name
        for name in (
            "b12_quad_local.route_guide",
            "b12_quad_local.congestion.rpt",
            "b12_quad_local_global_route.odb",
            "b12_quad_local_global_route.sdc",
        )
    )
    conditions = {
        ...
        and physical_auth.get("authorizes") == ["B12_ROUTE_AUTHORIZATION"],
        "selected_b12_policy": decision.get("decision") == "SELECT_B12_SIGNAL_LAYER_BOTTOM_ECO"
        and decision.get("selected_eco", {}).get("single_independent_variable") == "route_signal_layer_bottom"
        and decision.get("selected_eco", {}).get("before") == "met1"
        and decision.get("selected_eco", {}).get("after") == "met2",
        ...
        "b12_only_paths": "quad_local_b7" not in runner_text and "WBQ_B7" not in tcl_text,
    ...
    payload = {
        ...
        "inputs": {
            ...
            "b12_eco_decision": evidence(DECISION),
            "b12_physical_authorization": evidence(PHYSICAL_AUTH),
        ...
        "policy": {
            ...
            "service": "wbq-b12-global-route.service",
            "fail_closed": True,
        },
        "authorizes": ["B12_SINGLE_GLOBAL_ROUTE"] if passed else [],
        "next_stage": "B12_SINGLE_GLOBAL_ROUTE" if passed else None,
    }
    OUTPUT.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    print(f"B12_GLOBAL_ROUTE_AUTHORIZATION {payload['decision']}")
