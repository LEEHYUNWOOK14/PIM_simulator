#!/usr/bin/env python3
"""Issue a hash-pinned authorization for B22's sole global-route attempt."""
...
PHYSICAL_AUTH = PHYSICAL / "b22_route_pin_smoke_execution_report.json"
...
OUTPUT = REPORT / "b22_global_route_authorization.json"
...
def main() -> int:
    if OUTPUT.exists():
        raise FileExistsError(f"refusing to overwrite B22 route authorization: {OUTPUT}")
    ...
    route_outputs = [
        PHYSICAL / "b22_global_route_invocation.json",
        PHYSICAL / "b22_global_route_execution_report.json",
        PHYSICAL / "b22_global_route.log",
        REPORT / "b22_residual_congestion_analysis.json",
        REPORT / "b22_residual_congestion_analysis.md",
    ]
    artifact_root = Path("/dev/shm/wbq_b22_phase6_10/quad_local_b22")
    route_outputs.extend(
        artifact_root / name
        for name in (
            "b22_quad_local.route_guide",
            "b22_quad_local.congestion.rpt",
            "b22_quad_local_global_route.odb",
            "b22_quad_local_global_route.sdc",
        )
    )
    conditions = {
        ...
        and physical_auth.get("authorizes") == ["B22_ROUTE_AUTHORIZATION"],
        "selected_b22_policy": decision.get("decision") == "SELECT_B22_BUMP_ORIGIN_ECO"
        ...
        and decision.get("selected_eco", {}).get("after") == 500.0,
        ...
        "b22_only_paths": "quad_local_b7" not in runner_text and "WBQ_B7" not in tcl_text,
    ...
    payload = {
        ...
        "inputs": {
            ...
            "b22_eco_decision": evidence(DECISION),
            "b22_physical_authorization": evidence(PHYSICAL_AUTH),
        ...
        "policy": {
            ...
            "service": "wbq-b22-global-route.service",
            "fail_closed": True,
        },
        "authorizes": ["B22_SINGLE_GLOBAL_ROUTE"] if passed else [],
        "next_stage": "B22_SINGLE_GLOBAL_ROUTE" if passed else None,
    }
    OUTPUT.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    print(f"B22_GLOBAL_ROUTE_AUTHORIZATION {payload['decision']}")
