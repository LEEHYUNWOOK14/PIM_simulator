#!/usr/bin/env python3
"""Issue a hash-pinned authorization for B14's sole global-route attempt."""
...
PHYSICAL_AUTH = PHYSICAL / "b14_route_pin_smoke_execution_report.json"
...
OUTPUT = REPORT / "b14_global_route_authorization.json"
...
def main() -> int:
    if OUTPUT.exists():
        raise FileExistsError(f"refusing to overwrite B14 route authorization: {OUTPUT}")
    ...
    route_outputs = [
        PHYSICAL / "b14_global_route_invocation.json",
        PHYSICAL / "b14_global_route_execution_report.json",
        PHYSICAL / "b14_global_route.log",
        REPORT / "b14_residual_congestion_analysis.json",
        REPORT / "b14_residual_congestion_analysis.md",
    ]
    artifact_root = Path("/dev/shm/wbq_b14_phase6_10/quad_local_b14")
    route_outputs.extend(
        artifact_root / name
        for name in (
            "b14_quad_local.route_guide",
            "b14_quad_local.congestion.rpt",
            "b14_quad_local_global_route.odb",
            "b14_quad_local_global_route.sdc",
        )
    )
    conditions = {
        ...
        and physical_auth.get("authorizes") == ["B14_ROUTE_AUTHORIZATION"],
        "selected_b14_policy": decision.get("decision") == "SELECT_B14_BUMP_GRID_WIDER_ECO"
        and decision.get("selected_eco", {}).get("single_independent_variable") == "route_bump_pin_grid_columns"
        and decision.get("selected_eco", {}).get("before") == 33
        and decision.get("selected_eco", {}).get("after") == 17,
        ...
        "b14_only_paths": "quad_local_b7" not in runner_text and "WBQ_B7" not in tcl_text,
    ...
    payload = {
        ...
        "inputs": {
            ...
            "b14_eco_decision": evidence(DECISION),
            "b14_physical_authorization": evidence(PHYSICAL_AUTH),
        ...
        "policy": {
            ...
            "service": "wbq-b14-global-route.service",
            "fail_closed": True,
        },
        "authorizes": ["B14_SINGLE_GLOBAL_ROUTE"] if passed else [],
        "next_stage": "B14_SINGLE_GLOBAL_ROUTE" if passed else None,
    }
    OUTPUT.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    print(f"B14_GLOBAL_ROUTE_AUTHORIZATION {payload['decision']}")
