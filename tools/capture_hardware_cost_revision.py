...
from hardware_cost_evidence_contract import EvidenceContractError, require_valid_snapshot
from physical_feasibility_contract import pending_physical_feasibility, require_valid_physical_feasibility
...
def source_record(p: Path, role: str, claim_class="derived", tool=None, tool_version=None):
    ...
    return {"role":role,"path":locator,"sha256":digest(p),"bytes":p.stat().st_size,
            "claim_class":claim_class,"tool":tool,"tool_version":tool_version}
...
def capture(manifest_path,output_path):
    ...
    if manifest.get("schema_version")!=2: raise CaptureError("manifest schema_version must be 2")
    ...
    if contract.get("contract_version") != 2: raise CaptureError("evidence_contract.contract_version must be 2")
    calibration=manifest.get("calibration",{})
    required_axes={"synthesis","physical","power","thermal","timing","workload"}
    if set(calibration) != required_axes: raise CaptureError(f"calibration must define exactly {sorted(required_axes)}")
    toolchain=contract.get("toolchain",{})
    commit,dirty=git_info(); sources=[
        source_record(path(manifest_path),"manifest","assumed",toolchain.get("capture_tool"),toolchain.get("capture_tool_version")),
        source_record(mapped_path,"mapped_power","modeled",toolchain.get("power_tool"),toolchain.get("power_tool_version")),
        source_record(thermal_path,"thermal","modeled",toolchain.get("thermal_tool"),toolchain.get("thermal_tool_version")),
        source_record(workload_path,"gr00t_contract","assumed")]
    observations=[]
    for item in manifest.get("synthesis_observations",[]):
        ...
        sources.append(source_record(p,f"yosys:{item['name']}","derived",toolchain.get("synthesis_tool"),toolchain.get("synthesis_tool_version")))
        ...
        observation_calibration=item.get("calibration") or ("technology_mapped" if any(x is not None for x in (mapped_area,critical,slack)) else "synthesis_generic")
        observations.append({"name":item["name"],"precision":item["precision"],"category":item["category"],
            ...
            "critical_path_ns":critical,"slack_ns":slack,"calibration":observation_calibration,"claim_class":"derived","evidence":sources[-1]["path"],
    ...
    for block in mapped["blocks"]:
        ...
        block_metrics.append({"instance":block["instance"],"module":block["module"],"category":cat,"target":block["target"],
            ...
            "total_power_W":block["total_power_W"],"power_density_W_mm2":block["power_density_W_mm2"],"calibration":calibration["physical"],"claim_class":"modeled"})
    for row in category.values():
        ...
        row["physical_calibration"]=calibration["physical"] if row["blocks"] else "not_available"
        row["power_calibration"]=calibration["power"] if row["blocks"] else "not_available"
    metrics=workload.get("metrics",{}); throughput=number(metrics.get("throughput_ops_s"),"throughput")
    total_power=float(mapped["checks"]["mapped_power_W"])
    energy_eligible=calibration["power"] in {"activity_based","post_route","measured"}
    energy_op=total_power/throughput if throughput and energy_eligible else None
    ...
    workload_result={"status":workload["status"],"name":workload.get("workload",{}).get("name"),"precision":workload.get("workload",{}).get("precision"),
        ...
        "throughput_ops_s":throughput,"energy_per_op_J":energy_op,"calibration":calibration["workload"],"claim_class":"modeled" if workload["status"]!="pending" else "assumed",
        "note":("Energy/op remains null until activity-based or stronger power and throughput are both supplied."
                if energy_op is None else "Derived as total mapped power / throughput.")}
    physical_feasibility = pending_physical_feasibility()
    if manifest.get("physical_feasibility_json"):
        physical_path = path(manifest["physical_feasibility_json"])
        physical_feasibility = json.loads(physical_path.read_text(encoding="utf-8"))
        require_valid_physical_feasibility(physical_feasibility)
        sources.append(source_record(physical_path,"physical_feasibility","derived",toolchain.get("physical_tool"),toolchain.get("physical_tool_version")))
    result={"schema_version":2,"evidence_contract":{"contract_version":2,"status":"PROVISIONAL","claim_boundary":contract["claim_boundary"],
            "candidate_parameters":contract["candidate_parameters"],"toolchain":toolchain,"operating_point":contract["operating_point"]},
        "revision":{"id":manifest["revision_id"],"label":manifest["label"],"git_commit":commit,"working_tree_dirty":dirty,"captured_at":manifest["captured_at"]},
        "parameter_status":status,"calibration":calibration,
        ...
        "thermal":{"peak_temperature_K":number(thermal["peak_temperature_K"],"peak temperature",False),"peak_delta_K":number(thermal["peak_delta_K"],"peak delta",False),"hotspot":thermal["hotspot"],"calibration":calibration["thermal"],"claim_class":"modeled"},
    ...
    try: require_valid_snapshot(result)
    except EvidenceContractError as exc: raise CaptureError(str(exc)) from exc
