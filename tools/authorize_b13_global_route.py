#!/usr/bin/env python3
"""Issue a hash-pinned authorization for B13's sole global-route attempt."""
...
PHYSICAL_AUTH = PHYSICAL / "b13_route_pin_smoke_execution_report.json"
...
OUTPUT = REPORT / "b13_global_route_authorization.json"
...
def main() -> int:
    if OUTPUT.exists():
        raise FileExistsError(f"refusing to overwrite B13 route authorization: {OUTPUT}")
    ...
    route_outputs = [
        PHYSICAL / "b13_global_route_invocation.json",
        PHYSICAL / "b13_global_route_execution_report.json",
        PHYSICAL / "b13_global_route.log",
        REPORT / "b13_residual_congestion_analysis.json",
        REPORT / "b13_residual_congestion_analysis.md",
    ]
    artifact_root = Path("/dev/shm/wbq_b13_phase6_10/quad_local_b13")
    route_outputs.extend(
        artifact_root / name
        for name in (
            "b13_quad_local.route_guide",
            "b13_quad_local.congestion.rpt",
            "b13_quad_local_global_route.odb",
            "b13_quad_local_global_route.sdc",
        )
    )
    conditions = {
        ...
        and physical_auth.get("authorizes") == ["B13_ROUTE_AUTHORIZATION"],
        "selected_b13_policy": decision.get("decision") == "SELECT_B13_BUMP_GRID_WIDER_ECO"
        and decision.get("selected_eco", {}).get("single_independent_variable") == "route_bump_pin_grid_columns"
        and decision.get("selected_eco", {}).get("before") == 21
        and decision.get("selected_eco", {}).get("after") == 33,
        ...
        "b13_only_paths": "quad_local_b7" not in runner_text and "WBQ_B7" not in tcl_text,
    ...
    payload = {
        ...
        "inputs": {
            ...
            "b13_eco_decision": evidence(DECISION),
            "b13_physical_authorization": evidence(PHYSICAL_AUTH),
        ...
        "policy": {
            ...
            "service": "wbq-b13-global-route.service",
            "fail_closed": True,
        },
        "authorizes": ["B13_SINGLE_GLOBAL_ROUTE"] if passed else [],
        "next_stage": "B13_SINGLE_GLOBAL_ROUTE" if passed else None,
    }
    OUTPUT.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    print(f"B13_GLOBAL_ROUTE_AUTHORIZATION {payload['decision']}")
