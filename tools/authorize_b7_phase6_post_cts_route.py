#!/usr/bin/env python3
"""Hash-pin B7's sole post-CTS global-route invocation."""
...
CTS_MANIFEST = PHASE6 / "b7_phase6_cts_execution_report.json"
...
OUTPUT = PHASE6 / "b7_phase6_post_cts_route_authorization.json"
...
def main() -> int:
    if OUTPUT.exists():
        raise FileExistsError(f"refusing to overwrite B7 post-CTS route authorization: {OUTPUT}")
    ...
    route_root = Path("/dev/shm/wbq_b7_phase6_10/phase6_post_cts")
    prior_outputs = [
        PHASE6 / name
        for name in (
            "b7_phase6_post_cts_preflight.json", "b7_phase6_post_cts_global_route_invocation.json",
            "b7_phase6_post_cts_global_route_execution_report.json", "b7_phase6_post_cts_global_route.log",
            "b7_phase6_post_cts_route_audit.log", "b7_phase6_post_cts_congestion_analysis.json",
            "b7_phase6_post_cts_congestion_analysis.md",
        )
    ] + [
        route_root / name
        for name in (
            "b7_phase6_post_cts.route_guide", "b7_phase6_post_cts.congestion.rpt",
            "b7_phase6_post_cts_global_route.odb", "b7_phase6_post_cts_global_route.sdc",
        )
    ]
    conditions = {
        "cts_pass": cts.get("status") == "PASS",
        "cts_authorizes_post_route": "B7_PHASE6_POST_CTS_GLOBAL_ROUTE_AUTHORIZATION" in cts.get("authorizes", []),
    ...
    payload = {
        ...
        "policy": {
            "service": "wbq-b7-phase6-post-cts-route.service",
        ...
        "authorizes": ["B7_PHASE6_POST_CTS_GLOBAL_ROUTE_COMPUTE"] if passed else [],
        "next_stage": "B7_PHASE6_POST_CTS_GLOBAL_ROUTE_COMPUTE" if passed else None,
    }
    OUTPUT.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    print(f"B7_PHASE6_POST_CTS_ROUTE_AUTHORIZATION {payload['decision']}")
