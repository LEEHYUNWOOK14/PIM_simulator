#!/usr/bin/env python3
"""Fail-closed Phase 10 audit for the complete B3 research-artifact chain."""
...
PHASE9_ROOT = Path(os.environ.get("WBQ_B3_PHASE9_ROOT", "/dev/shm/wbq_b3_phase6_10/phase9"))
EVIDENCE = B3 / "phase10"
PREFLIGHT = EVIDENCE / "b3_phase10_preflight.json"
HASH_MANIFEST = EVIDENCE / "b3_phase6_phase10_artifact_hash_manifest.json"
COMPLETION = EVIDENCE / "b3_phase10_completion_report.json"
MARKDOWN = EVIDENCE / "b3_phase10_completion_report.md"

CHAIN = {
    "root_cause": B3 / "b3_cheap_root_cause_analysis.json",
    "eco_decision": B3 / "b3_eco_decision.json",
    "cheap_gate": B3 / "cheap_gate_manifest.json",
    "physical_authorization": B3 / "b3_physical_authorization.json",
    "placement": B3 / "physical/b3_placement_execution_report.json",
    "global_route": B3 / "physical/b3_global_route_execution_report.json",
    "congestion_analysis": B3 / "b3_residual_congestion_analysis.json",
    "phase6_gate": B3 / "phase6_decision_gate.json",
    "phase6_cts": B3 / "phase6/b3_phase6_cts_execution_report.json",
    "phase6_post_cts": B3 / "phase6/b3_phase6_post_cts_global_route_execution_report.json",
    "phase7_detailed_route": B3 / "phase7/b3_phase7_detailed_route_execution_report.json",
    "phase7_rtl_gds": B3 / "phase7/b3_phase7_rtl_gds_execution_report.json",
    "phase8_overlay": B3 / "phase8/b3_phase8_overlay_execution_report.json",
    "phase9_final_gds": B3 / "phase9/b3_phase9_final_gds_execution_report.json",
}
...
def main() -> int:
    ...
    if phase9.get("status") != "PASS" or "B3_PHASE10_COMPLETION_AUDIT" not in phase9.get("authorizes", []):
        raise RuntimeError("Phase 9 does not authorize Phase 10")

    generated = datetime.now(timezone.utc).isoformat()
    with PREFLIGHT.open("x", encoding="utf-8") as stream:
        json.dump({
            "schema_version": 1, "phase": 10, "variant": "B3_PHASE10",
            "status": "PASS", "generated_at_utc": generated,
            "phase9_input": artifact(CHAIN["phase9_final_gds"]),
            "openroad_processes_before": [],
            "authorizes": ["B3_PHASE10_COMPLETION_AUDIT_COMPUTE"],
            "next_stage": "B3_PHASE10_COMPLETION_AUDIT_COMPUTE",
        }, stream, indent=2)
        stream.write("\n")
    ...
    phase_checks = {
        ...
        and docs["phase6_gate"].get("authorizes") == ["B3_PHASE6_CTS"],
    ...
    phase_checks["b3_zero_congestion"] = congestion.get("rrr_residual") == 0 and congestion.get("overflow_edges") == 0
    ...
    clean_parent = Path("/dev/shm/wbq_b3_phase6_10")
    clean_parent.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(prefix="b3_phase10_clean_", dir=clean_parent) as temp_name:
        temp = Path(temp_name)
    ...
    hash_payload = {
        ...
        "variant": "B3_PHASE6_PHASE10", "artifacts": [artifact(path) for path in unique],
    ...
    completion = {
        "schema_version": 1, "generated_at_utc": datetime.now(timezone.utc).isoformat(),
        "phase": 10, "variant": "B3", "status": "PASS" if all_pass else "FAIL",
        ...
        "metrics": {"b3_rrr_residual": congestion.get("rrr_residual"),
                    "b3_overflow_edges": congestion.get("overflow_edges"),
    ...
    MARKDOWN.write_text(
        ...
        f"- B3 residual/overflow: `{congestion.get('rrr_residual')}/{congestion.get('overflow_edges')}`\n"
    ...
    print(f"WBQ_B3_PHASE10_COMPLETION {completion['status']} gds_sha256={original_hash}")
