#!/usr/bin/env python3
"""Issue a one-shot, hash-pinned authorization for B6 detailed route."""
...
INPUT_ROOT = Path(
    os.environ.get(
        "WBQ_B6_PHASE6_ROUTE_ROOT", "/dev/shm/wbq_b6_phase6_10/phase6_post_cts"
    )
)
SOURCE = PHASE6 / "b6_phase6_post_cts_global_route_execution_report.json"
ODB = INPUT_ROOT / "b6_phase6_post_cts_global_route.odb"
SDC = INPUT_ROOT / "b6_phase6_post_cts_global_route.sdc"
OUTPUT = PHASE7 / "b6_phase7_detailed_route_authorization.json"

PINNED_TOOLS = {
    "authorization_tool": ROOT / "tools/authorize_b6_phase7_detailed_route.py",
    "runner": ROOT / "verification/groot_normalization/run_wbq_b6_phase7_detailed_route.sh",
    "route_tcl": ROOT / "verification/groot_normalization/wbq_b6_phase7_detailed_route.tcl",
...
REFUSE_IF_PRESENT = (
    PHASE7 / "b6_phase7_preflight.json",
    PHASE7 / "b6_phase7_detailed_route_invocation.json",
    PHASE7 / "b6_phase7_detailed_route_execution_report.json",
    PHASE7 / "b6_phase7_detailed_route.log",
)
...
def main() -> int:
    ...
    payload = {
        "schema_version": 1,
        "phase": 7,
        "variant": "B6_PHASE7_DETAILED_ROUTE",
        ...
        "resource_contract": {
            ...
            "basis": (
                "B2/B6 ODBs are approximately 3.3-4.4 GiB; 12 GiB retains "
                "headroom for ODB, DEF, reports, and logs in the bounded tmpfs."
            ),
        },
        "invocation_limit": 1,
        "authorizes": ["B6_PHASE7_DETAILED_ROUTE_COMPUTE"] if passed else [],
        "next_stage": "B6_PHASE7_DETAILED_ROUTE_COMPUTE" if passed else None,
    }
    with OUTPUT.open("x", encoding="utf-8") as stream:
        json.dump(payload, stream, indent=2)
        stream.write("\n")
    print(f"WBQ_B6_PHASE7_DETAILED_ROUTE_AUTHORIZATION {payload['decision']}")
