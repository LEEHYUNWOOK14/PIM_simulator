#!/usr/bin/env python3
"""Evaluate B16's four fail-closed Phase 6 release conditions."""
...
ROUTE_MANIFEST = PHYSICAL / "b16_global_route_execution_report.json"
ANALYSIS = REPORT / "b16_residual_congestion_analysis.json"
...
def main() -> int:
    ...
    payload = {
        ...
        "reason": (
            "B16 has zero RRR residual, zero numeric overflow edges, matching artifact hashes, and an explicit Phase 6 PASS."
            if passed
        ...
        "authorizes": ["B16_PHASE6_CTS"] if passed else [],
        "next_stage": "B16_PHASE6_CTS" if passed else None,
        "blocked_recovery": None if passed else "Seal B16 without reroute; create B16 with one evidence-based ECO and re-run smoke plus authorization.",
    }
    OUTPUT.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    print(
        f"B16_PHASE6_STRICT_GATE {payload['decision']} residual={residual} "
