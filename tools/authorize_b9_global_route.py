#!/usr/bin/env python3
"""Issue a hash-pinned authorization for B9's sole global-route attempt."""
...
NUMERIC_ANALYSIS = PHYSICAL / "b9_placement_numeric_analysis.json"
DECISION = REPORT / "b9_eco_decision.json"
PHYSICAL_AUTH = REPORT / "b9_physical_authorization.json"
...
OUTPUT = REPORT / "b9_global_route_authorization.json"
...
def main() -> int:
    if OUTPUT.exists():
        raise FileExistsError(f"refusing to overwrite B9 route authorization: {OUTPUT}")
    ...
    route_outputs = [
        PHYSICAL / "b9_global_route_invocation.json",
        PHYSICAL / "b9_global_route_execution_report.json",
        PHYSICAL / "b9_global_route.log",
        REPORT / "b9_residual_congestion_analysis.json",
        REPORT / "b9_residual_congestion_analysis.md",
    ]
    artifact_root = Path("/dev/shm/wbq_b9_phase6_10/quad_local_b9")
    route_outputs.extend(
        artifact_root / name
        for name in (
            "b9_quad_local.route_guide",
            "b9_quad_local.congestion.rpt",
            "b9_quad_local_global_route.odb",
            "b9_quad_local_global_route.sdc",
        )
    )
    conditions = {
        ...
        "selected_b9_policy": decision.get("decision") == "SELECT_B9_SEVEN_ADDITIONAL_ANCHORS_ECO"
        ...
        "b9_only_paths": "quad_local_b7" not in runner_text and "WBQ_B7" not in tcl_text,
    ...
    payload = {
        ...
        "inputs": {
            ...
            "b9_eco_decision": evidence(DECISION),
            "b9_physical_authorization": evidence(PHYSICAL_AUTH),
        ...
        "policy": {
            ...
            "service": "wbq-b9-global-route.service",
            "fail_closed": True,
        },
        "authorizes": ["B9_SINGLE_GLOBAL_ROUTE"] if passed else [],
        "next_stage": "B9_SINGLE_GLOBAL_ROUTE" if passed else None,
    }
    OUTPUT.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    print(f"B9_GLOBAL_ROUTE_AUTHORIZATION {payload['decision']}")
