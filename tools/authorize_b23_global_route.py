#!/usr/bin/env python3
"""Issue a hash-pinned authorization for B23's sole global-route attempt."""
...
PHYSICAL_AUTH = PHYSICAL / "b23_route_pin_smoke_execution_report.json"
...
OUTPUT = REPORT / "b23_global_route_authorization.json"
...
def main() -> int:
    if OUTPUT.exists():
        raise FileExistsError(f"refusing to overwrite B23 route authorization: {OUTPUT}")
    ...
    route_outputs = [
        PHYSICAL / "b23_global_route_invocation.json",
        PHYSICAL / "b23_global_route_execution_report.json",
        PHYSICAL / "b23_global_route.log",
        REPORT / "b23_residual_congestion_analysis.json",
        REPORT / "b23_residual_congestion_analysis.md",
    ]
    artifact_root = Path("/dev/shm/wbq_b23_phase6_10/quad_local_b23")
    route_outputs.extend(
        artifact_root / name
        for name in (
            "b23_quad_local.route_guide",
            "b23_quad_local.congestion.rpt",
            "b23_quad_local_global_route.odb",
            "b23_quad_local_global_route.sdc",
        )
    )
    conditions = {
        ...
        and physical_auth.get("authorizes") == ["B23_ROUTE_AUTHORIZATION"],
        "selected_b23_policy": decision.get("decision") == "SELECT_B23_BUMP_ORIGIN_ECO"
        ...
        and decision.get("selected_eco", {}).get("after") == 750.0,
        ...
        "b23_only_paths": "quad_local_b7" not in runner_text and "WBQ_B7" not in tcl_text,
    ...
    payload = {
        ...
        "inputs": {
            ...
            "b23_eco_decision": evidence(DECISION),
            "b23_physical_authorization": evidence(PHYSICAL_AUTH),
        ...
        "policy": {
            ...
            "service": "wbq-b23-global-route.service",
            "fail_closed": True,
        },
        "authorizes": ["B23_SINGLE_GLOBAL_ROUTE"] if passed else [],
        "next_stage": "B23_SINGLE_GLOBAL_ROUTE" if passed else None,
    }
    OUTPUT.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    print(f"B23_GLOBAL_ROUTE_AUTHORIZATION {payload['decision']}")
