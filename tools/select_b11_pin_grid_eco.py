#!/usr/bin/env python3
"""Seal B9 congestion and select one evidence-based B11 route-pin ECO."""
...
def main() -> int:
    decision_path = B11 / "b11_eco_decision.json"
    markdown_path = B11 / "b11_eco_decision.md"
    if decision_path.exists() or markdown_path.exists():
        raise FileExistsError("refusing to overwrite B11 ECO decision")
    ...
    if not (
        ...
        raise RuntimeError("B9 evidence does not support the B11 route-pin-grid ECO")

    B11.mkdir(parents=True, exist_ok=True)
    payload = {
        "schema_version": 1,
        "generated_at_utc": datetime.now(timezone.utc).isoformat(),
        "variant": "B11",
        "decision": "SELECT_B11_ROUTE_PIN_GRID_COLUMNS_ECO",
        ...
        "selected_eco": {
            ...
            "before": 25,
            "after": 21,
            "reason": "B9's direct parser found 448 residual violations and 346 overflow edges; 338 hotspots are in the central corridor and 289 include top-level I/O/other. Spread the fixed 565 bump terms across 21 rather than 25 columns while preserving RTL, netlist, legal placement, SDC, and route iteration policy.",
        },
        "unchanged_contract": {
            "rtl": "byte-identical",
        ...
        "authorizes": [],
        "next_stage": "B11_SMOKE",
    }
    decision_path.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    markdown_path.write_text(
        "# B11 route pin-grid ECO\n\n"
        f"Generated: `{payload['generated_at_utc']}`\n\n"
        "B9 completed its single global-route invocation but strict Phase 6 remained blocked with residual 448 and 346 overflow edges. Direct hotspot analysis found 338 central-corridor hotspots, 289 with top-level I/O contribution. B11 changes exactly one route variable: bump-pin grid columns 25 to 21. RTL, netlist, legal placement, SDC, layers, fanout policy, and one-iteration route contract remain unchanged.\n",
        encoding="utf-8",
    )
    print(f"B11_ECO_DECISION SELECTED output={decision_path}")
    return 0
