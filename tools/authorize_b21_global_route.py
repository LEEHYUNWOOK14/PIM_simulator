#!/usr/bin/env python3
"""Issue a hash-pinned authorization for B21's sole global-route attempt."""
...
PHYSICAL_AUTH = PHYSICAL / "b21_route_pin_smoke_execution_report.json"
...
OUTPUT = REPORT / "b21_global_route_authorization.json"
...
def main() -> int:
    if OUTPUT.exists():
        raise FileExistsError(f"refusing to overwrite B21 route authorization: {OUTPUT}")
    ...
    route_outputs = [
        PHYSICAL / "b21_global_route_invocation.json",
        PHYSICAL / "b21_global_route_execution_report.json",
        PHYSICAL / "b21_global_route.log",
        REPORT / "b21_residual_congestion_analysis.json",
        REPORT / "b21_residual_congestion_analysis.md",
    ]
    artifact_root = Path("/dev/shm/wbq_b21_phase6_10/quad_local_b21")
    route_outputs.extend(
        artifact_root / name
        for name in (
            "b21_quad_local.route_guide",
            "b21_quad_local.congestion.rpt",
            "b21_quad_local_global_route.odb",
            "b21_quad_local_global_route.sdc",
        )
    )
    conditions = {
        ...
        and physical_auth.get("authorizes") == ["B21_ROUTE_AUTHORIZATION"],
        "selected_b21_policy": decision.get("decision") == "SELECT_B21_BUMP_ORIGIN_ECO"
        ...
        "b21_only_paths": "quad_local_b7" not in runner_text and "WBQ_B7" not in tcl_text,
    ...
    payload = {
        ...
        "inputs": {
            ...
            "b21_eco_decision": evidence(DECISION),
            "b21_physical_authorization": evidence(PHYSICAL_AUTH),
        ...
        "policy": {
            ...
            "service": "wbq-b21-global-route.service",
            "fail_closed": True,
        },
        "authorizes": ["B21_SINGLE_GLOBAL_ROUTE"] if passed else [],
        "next_stage": "B21_SINGLE_GLOBAL_ROUTE" if passed else None,
    }
    OUTPUT.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    print(f"B21_GLOBAL_ROUTE_AUTHORIZATION {payload['decision']}")
