...
evidence="$root/reports/groot_normalization/quad_local_b3/phase8"
...
tsv_csv="$phase8_root/b3_tsv_connectivity.csv"
transform="$phase8_root/b3_floorplan_transform.json"
schema_validation="$phase8_root/b3_overlay_schema_validation.json"
...
preflight="$evidence/b3_phase8_preflight.json"
manifest="$evidence/b3_phase8_overlay_execution_report.json"
report="$evidence/b3_phase8_overlay_execution_report.md"
...
if manifest.get("status") != "PASS" or "B3_PHASE8_OVERLAY" not in manifest.get("authorizes", []):
    raise SystemExit("B3 Phase 7 RTL GDS does not authorize Phase 8")
if sha(pathlib.Path(sys.argv[2])) != manifest["gds"]["sha256"]:
    raise SystemExit("B3 Phase 8 RTL GDS hash mismatch")
print("WBQ_B3_PHASE8_INPUT_GATE PASS")
PY
for path in "$floorplan" "$tsv_csv" "$transform" "$schema_validation" "$build_gds" "$build_lyp" \
  ...
  if [[ -e "$path" ]]; then echo "existing B3 Phase 8 artifact prevents overwrite: $path" >&2; exit 4; fi
done
...
payload = {"schema_version": 1, "phase": 8, "variant": "B3_PHASE8_OVERLAY",
           ...
           "authorizes": ["B3_PHASE8_OVERLAY_BUILD"], "next_stage": "B3_PHASE8_OVERLAY_BUILD"}
...
if transform.get("anchor_count") != 2 or len(anchors) != 2:
    raise SystemExit("B3 Phase 8 requires exactly two anchors")
if any(float(item.get("error_um", -1)) != 0.0 for item in anchors):
    raise SystemExit("B3 Phase 8 anchor error is nonzero")
if transform.get("target_rtl_gds_bbox_um") != rtl.get("geometry", {}).get("bbox_um"):
    raise SystemExit("B3 Phase 8 target bbox mismatch")
print("WBQ_B3_PHASE8_ANCHOR_GATE PASS anchors=2 max_error_um=0")
PY
...
payload = {"schema_version": 1, "phase": 8, "variant": "B3_PHASE8_OVERLAY",
           ...
           "authorizes": ["B3_PHASE9_FINAL_GDS_MERGE"],
           "next_stage": "B3_PHASE9_FINAL_GDS_MERGE",
...
pathlib.Path(report).write_text(
    ...
    encoding="utf-8")
PY
echo "WBQ_B3_PHASE8_OVERLAY PASS report=$manifest"
