...
cd "$root"
phase7_evidence="$root/reports/groot_normalization/quad_local_b5/phase7"
phase8_evidence="$root/reports/groot_normalization/quad_local_b5/phase8"
evidence="$root/reports/groot_normalization/quad_local_b5/phase9"
phase7_root="${WBQ_B5_PHASE7_ROOT:-/dev/shm/wbq_b5_phase6_10/phase7}"
phase8_root="${WBQ_B5_PHASE8_ROOT:-/dev/shm/wbq_b5_phase6_10/phase8}"
phase9_root="${WBQ_B5_PHASE9_ROOT:-/dev/shm/wbq_b5_phase6_10/phase9}"
rtl_manifest="$phase7_evidence/b5_phase7_rtl_gds_execution_report.json"
overlay_manifest="$phase8_evidence/b5_phase8_overlay_execution_report.json"
rtl_gds="$phase7_root/b5_phase7_routed.gds"
overlay_gds="$phase8_root/b5_tsv_bump_overlay.gds"
overlay_lyp="$phase8_root/b5_tsv_bump_overlay.lyp"
floorplan="$phase8_root/b5_floorplan_manifest.json"
recipe="$phase9_root/b5_final_gds_merge_recipe.json"
final_dir="$phase9_root/final"
gds="$final_dir/merged_final_physical.gds"
lyp="$final_dir/merged_final_physical.lyp"
merge_report="$phase9_root/b5_merged_final_physical_report.json"
png="$phase9_root/b5_klayout_fixed_camera.png"
preflight="$evidence/b5_phase9_preflight.json"
log="$evidence/b5_phase9_merge_render.log"
manifest="$evidence/b5_phase9_final_gds_execution_report.json"
...
for path in "$recipe" "$gds" "$lyp" "$merge_report" "$png" "$preflight" "$log" "$manifest"; do
  if [[ -e "$path" ]]; then echo "existing B5 Phase 9 artifact prevents overwrite: $path" >&2; exit 4; fi
done
...
if rtl.get("status") != "PASS" or "B5_PHASE8_OVERLAY" not in rtl.get("authorizes", []):
    raise SystemExit("B5 RTL GDS manifest is not an authorizing PASS")
if overlay.get("status") != "PASS" or "B5_PHASE9_FINAL_GDS_MERGE" not in overlay.get("authorizes", []):
    raise SystemExit("B5 overlay manifest is not an authorizing PASS")
if rtl["gds"]["sha256"] != sha(rtl_gds): raise SystemExit("B5 RTL GDS hash mismatch")
for key, path in (("overlay_gds", overlay_gds), ("overlay_lyp", overlay_lyp), ("floorplan_manifest", floorplan)):
    if overlay["artifacts"][key]["sha256"] != sha(path): raise SystemExit(f"B5 Phase 9 hash mismatch: {key}")
bbox = rtl.get("geometry", {}).get("bbox_um")
if bbox != overlay.get("die_bbox_um") or len(bbox or []) != 4: raise SystemExit("B5 RTL/overlay bbox mismatch")
...
payload = {"schema_version": 1, "phase": 9, "variant": "B5_PHASE9_FINAL_GDS",
           ...
           "authorizes": ["B5_PHASE9_MERGE_RENDER"], "next_stage": "B5_PHASE9_MERGE_RENDER"}
with open(preflight, "x", encoding="utf-8") as stream:
    json.dump(payload, stream, indent=2); stream.write("\n")
PY

{
  echo "WBQ_B5_PHASE9_START_UTC=$start"
  echo "WBQ_B5_PHASE9_MERGE_INVOCATION_COUNT=1"
  echo "WBQ_B5_PHASE9_RENDER_INVOCATION_COUNT=1"
} > "$log"
...
if failed: raise SystemExit("B5 Phase 9 merge gate failed: " + ", ".join(failed))
print("WBQ_B5_PHASE9_MERGE_GATE PASS")
PY

{
  echo "WBQ_B5_PHASE9_RENDER_GDS_SHA256=$(sha256sum "$gds" | awk '{print $1}')"
  echo "WBQ_B5_PHASE9_RENDER_LYP_SHA256=$(sha256sum "$lyp" | awk '{print $1}')"
  STOB_FINAL_GDS="$gds" STOB_FINAL_LYP="$lyp" STOB_FINAL_PNG="$png" \
    klayout -zz -r tools/render_wbq_final_gds.py
  echo "WBQ_B5_PHASE9_RENDER_PNG_SHA256=$(sha256sum "$png" | awk '{print $1}')"
} >> "$log" 2>&1
test -s "$png"
grep -q '^KLAYOUT_WBQ_FINAL_RENDER PASS ' "$log"
end="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
echo "WBQ_B5_PHASE9_END_UTC=$end" >> "$log"
...
payload = {"schema_version": 1, "phase": 9, "variant": "B5_PHASE9_FINAL_GDS",
           ...
           "authorizes": ["B5_PHASE10_COMPLETION_AUDIT"],
           "next_stage": "B5_PHASE10_COMPLETION_AUDIT",
...
echo "WBQ_B5_PHASE9_FINAL_GDS PASS report=$manifest"
