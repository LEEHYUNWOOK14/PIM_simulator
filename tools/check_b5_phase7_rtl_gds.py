#!/usr/bin/env python3
"""Independently reopen and gate the B5 Phase 7 routed RTL GDS."""
...
def main() -> int:
    ...
    logged = re.findall(r"^WBQ_B5_RTL_GDS_OUTPUT_SHA256=(\S+)$", log, re.MULTILINE)
    layout = kdb.Layout()
    ...
    checks = {
        ...
        and "B5_PHASE7_RTL_GDS_STREAMOUT" in source.get("authorizes", []),
        "streamout_exit_zero": "WBQ_B5_RTL_GDS_EXIT_CODE=0" in log,
    ...
    payload = {
        "schema_version": 1, "phase": 7, "variant": "B5_PHASE7_RTL_GDS",
        ...
        "authorizes": ["B5_PHASE8_OVERLAY"] if passed else [],
        "next_stage": "B5_PHASE8_OVERLAY" if passed else None,
    ...
    print(f"WBQ_B5_PHASE7_RTL_GDS_READBACK {payload['status']} sha256={actual_hash}")
