#!/usr/bin/env python3
"""Issue a hash-pinned authorization for B24's sole global-route attempt."""
...
PHYSICAL_AUTH = PHYSICAL / "b24_route_pin_smoke_execution_report.json"
...
OUTPUT = REPORT / "b24_global_route_authorization.json"
...
def main() -> int:
    if OUTPUT.exists():
        raise FileExistsError(f"refusing to overwrite B24 route authorization: {OUTPUT}")
    ...
    route_outputs = [
        PHYSICAL / "b24_global_route_invocation.json",
        PHYSICAL / "b24_global_route_execution_report.json",
        PHYSICAL / "b24_global_route.log",
        REPORT / "b24_residual_congestion_analysis.json",
        REPORT / "b24_residual_congestion_analysis.md",
    ]
    artifact_root = Path("/dev/shm/wbq_b24_phase6_10/quad_local_b24")
    route_outputs.extend(
        artifact_root / name
        for name in (
            "b24_quad_local.route_guide",
            "b24_quad_local.congestion.rpt",
            "b24_quad_local_global_route.odb",
            "b24_quad_local_global_route.sdc",
        )
    )
    conditions = {
        ...
        and physical_auth.get("authorizes") == ["B24_ROUTE_AUTHORIZATION"],
        "selected_b24_policy": decision.get("decision") == "SELECT_B24_BUMP_ORIGIN_ECO"
        and decision.get("selected_eco", {}).get("single_independent_variable") == "route_bump_pin_y_origin"
        and decision.get("selected_eco", {}).get("before") == 1000.0
        and decision.get("selected_eco", {}).get("after") == 1250.0,
        ...
        "b24_only_paths": "quad_local_b7" not in runner_text and "WBQ_B7" not in tcl_text,
    ...
    payload = {
        ...
        "inputs": {
            ...
            "b24_eco_decision": evidence(DECISION),
            "b24_physical_authorization": evidence(PHYSICAL_AUTH),
        ...
        "policy": {
            ...
            "service": "wbq-b24-global-route.service",
            "fail_closed": True,
        },
        "authorizes": ["B24_SINGLE_GLOBAL_ROUTE"] if passed else [],
        "next_stage": "B24_SINGLE_GLOBAL_ROUTE" if passed else None,
    }
    OUTPUT.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    print(f"B24_GLOBAL_ROUTE_AUTHORIZATION {payload['decision']}")
