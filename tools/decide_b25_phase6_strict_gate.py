#!/usr/bin/env python3
"""Evaluate B25's four fail-closed Phase 6 release conditions."""
...
ROUTE_MANIFEST = PHYSICAL / "b25_global_route_execution_report.json"
ANALYSIS = REPORT / "b25_residual_congestion_analysis.json"
...
def main() -> int:
    ...
    input_hashes_match = (
        ...
        and route.get("global_route_invocations") == 1
    ...
    conditions = {
        "residual_congestion_zero": {"required": 0, "actual": residual, "result": "PASS" if residual == 0 else "FAIL"},
        "overflow_edges_zero": {"required": 0, "actual": overflow_edges, "result": "PASS" if overflow_edges == 0 else "FAIL"},
        "input_artifact_hashes_match": {"required": True, "actual": input_hashes_match, "result": "PASS" if input_hashes_match else "FAIL"},
        "explicit_phase6_pass": {"required": True, "actual": explicit_pass, "result": "PASS" if explicit_pass else "FAIL"},
    }
    ...
    payload = {
        ...
        "reason": "B25 satisfies strict zero congestion and hash conditions." if passed else "One or more strict zero-congestion/hash conditions failed; Phase 6 remains fail-closed.",
        ...
        "global_route_invocation_count": route.get("global_route_invocations"),
        "authorizes": ["B25_PHASE6_CTS"] if passed else [],
        "next_stage": "B25_PHASE6_CTS" if passed else None,
        "blocked_recovery": None if passed else "Seal B25 without reroute; create a new structural variant with fresh smoke and authorization.",
    }
    OUTPUT.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    print(f"B25_PHASE6_STRICT_GATE {payload['decision']} residual={residual} overflow_edges={overflow_edges} authorizes={payload['authorizes']}")
