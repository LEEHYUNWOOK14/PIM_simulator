#!/usr/bin/env python3
"""Issue a hash-pinned authorization for B10's sole global-route attempt."""
...
PHYSICAL_AUTH = PHYSICAL / "b10_route_pin_smoke_execution_report.json"
...
OUTPUT = REPORT / "b10_global_route_authorization.json"
...
def main() -> int:
    if OUTPUT.exists():
        raise FileExistsError(f"refusing to overwrite B10 route authorization: {OUTPUT}")
    ...
    route_outputs = [
        PHYSICAL / "b10_global_route_invocation.json",
        PHYSICAL / "b10_global_route_execution_report.json",
        PHYSICAL / "b10_global_route.log",
        REPORT / "b10_residual_congestion_analysis.json",
        REPORT / "b10_residual_congestion_analysis.md",
    ]
    artifact_root = Path("/dev/shm/wbq_b10_phase6_10/quad_local_b10")
    route_outputs.extend(
        artifact_root / name
        for name in (
            "b10_quad_local.route_guide",
            "b10_quad_local.congestion.rpt",
            "b10_quad_local_global_route.odb",
            "b10_quad_local_global_route.sdc",
        )
    )
    conditions = {
        ...
        and physical_auth.get("authorizes") == ["B10_ROUTE_AUTHORIZATION"],
        "selected_b10_policy": decision.get("decision") == "SELECT_B10_ROUTE_PIN_GRID_COLUMNS_ECO"
        ...
        and decision.get("selected_eco", {}).get("after") == 29,
        ...
        "b10_only_paths": "quad_local_b7" not in runner_text and "WBQ_B7" not in tcl_text,
    ...
    payload = {
        ...
        "inputs": {
            ...
            "b10_eco_decision": evidence(DECISION),
            "b10_physical_authorization": evidence(PHYSICAL_AUTH),
        ...
        "policy": {
            ...
            "service": "wbq-b10-global-route.service",
            "fail_closed": True,
        },
        "authorizes": ["B10_SINGLE_GLOBAL_ROUTE"] if passed else [],
        "next_stage": "B10_SINGLE_GLOBAL_ROUTE" if passed else None,
    }
    OUTPUT.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    print(f"B10_GLOBAL_ROUTE_AUTHORIZATION {payload['decision']}")
