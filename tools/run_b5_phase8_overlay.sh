...
evidence="$root/reports/groot_normalization/quad_local_b5/phase8"
...
tsv_csv="$phase8_root/b5_tsv_connectivity.csv"
transform="$phase8_root/b5_floorplan_transform.json"
schema_validation="$phase8_root/b5_overlay_schema_validation.json"
...
preflight="$evidence/b5_phase8_preflight.json"
manifest="$evidence/b5_phase8_overlay_execution_report.json"
report="$evidence/b5_phase8_overlay_execution_report.md"
...
if manifest.get("status") != "PASS" or "B5_PHASE8_OVERLAY" not in manifest.get("authorizes", []):
    raise SystemExit("B5 Phase 7 RTL GDS does not authorize Phase 8")
if sha(pathlib.Path(sys.argv[2])) != manifest["gds"]["sha256"]:
    raise SystemExit("B5 Phase 8 RTL GDS hash mismatch")
print("WBQ_B5_PHASE8_INPUT_GATE PASS")
PY
for path in "$floorplan" "$tsv_csv" "$transform" "$schema_validation" "$build_gds" "$build_lyp" \
  ...
  if [[ -e "$path" ]]; then echo "existing B5 Phase 8 artifact prevents overwrite: $path" >&2; exit 4; fi
done
...
payload = {"schema_version": 1, "phase": 8, "variant": "B5_PHASE8_OVERLAY",
           ...
           "authorizes": ["B5_PHASE8_OVERLAY_BUILD"], "next_stage": "B5_PHASE8_OVERLAY_BUILD"}
...
if transform.get("anchor_count") != 2 or len(anchors) != 2:
    raise SystemExit("B5 Phase 8 requires exactly two anchors")
if any(float(item.get("error_um", -1)) != 0.0 for item in anchors):
    raise SystemExit("B5 Phase 8 anchor error is nonzero")
if transform.get("target_rtl_gds_bbox_um") != rtl.get("geometry", {}).get("bbox_um"):
    raise SystemExit("B5 Phase 8 target bbox mismatch")
print("WBQ_B5_PHASE8_ANCHOR_GATE PASS anchors=2 max_error_um=0")
PY
...
payload = {"schema_version": 1, "phase": 8, "variant": "B5_PHASE8_OVERLAY",
           ...
           "authorizes": ["B5_PHASE9_FINAL_GDS_MERGE"],
           "next_stage": "B5_PHASE9_FINAL_GDS_MERGE",
...
pathlib.Path(report).write_text(
    ...
    encoding="utf-8")
PY
echo "WBQ_B5_PHASE8_OVERLAY PASS report=$manifest"
