#!/usr/bin/env python3
"""Issue a hash-pinned authorization for B17's sole global-route attempt."""
...
PHYSICAL_AUTH = PHYSICAL / "b17_route_pin_smoke_execution_report.json"
...
OUTPUT = REPORT / "b17_global_route_authorization.json"
...
def main() -> int:
    if OUTPUT.exists():
        raise FileExistsError(f"refusing to overwrite B17 route authorization: {OUTPUT}")
    ...
    route_outputs = [
        PHYSICAL / "b17_global_route_invocation.json",
        PHYSICAL / "b17_global_route_execution_report.json",
        PHYSICAL / "b17_global_route.log",
        REPORT / "b17_residual_congestion_analysis.json",
        REPORT / "b17_residual_congestion_analysis.md",
    ]
    artifact_root = Path("/dev/shm/wbq_b17_phase6_10/quad_local_b17")
    route_outputs.extend(
        artifact_root / name
        for name in (
            "b17_quad_local.route_guide",
            "b17_quad_local.congestion.rpt",
            "b17_quad_local_global_route.odb",
            "b17_quad_local_global_route.sdc",
        )
    )
    conditions = {
        ...
        and physical_auth.get("authorizes") == ["B17_ROUTE_AUTHORIZATION"],
        "selected_b17_policy": decision.get("decision") == "SELECT_B17_BUMP_PITCH_ECO"
        and decision.get("selected_eco", {}).get("single_independent_variable") == "route_bump_pin_pitch"
        and decision.get("selected_eco", {}).get("before") == 250.0
        and decision.get("selected_eco", {}).get("after") == 300.0,
        ...
        "b17_only_paths": "quad_local_b7" not in runner_text and "WBQ_B7" not in tcl_text,
    ...
    payload = {
        ...
        "inputs": {
            ...
            "b17_eco_decision": evidence(DECISION),
            "b17_physical_authorization": evidence(PHYSICAL_AUTH),
        ...
        "policy": {
            ...
            "service": "wbq-b17-global-route.service",
            "fail_closed": True,
        },
        "authorizes": ["B17_SINGLE_GLOBAL_ROUTE"] if passed else [],
        "next_stage": "B17_SINGLE_GLOBAL_ROUTE" if passed else None,
    }
    OUTPUT.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    print(f"B17_GLOBAL_ROUTE_AUTHORIZATION {payload['decision']}")
