#!/usr/bin/env python3
"""Create compact numeric evidence from the completed B7 placement log."""
...
def main() -> int:
    ...
    locked = re.findall(
        r"^WBQ_B7_ANCHOR_LOCKED name=\{(.+?)\} origin_dbu=\{(\d+) (\d+)\} orient=\{(\S+)\}$",
        text,
        re.MULTILINE,
    )
    verified = re.findall(
        r"^WBQ_B7_ANCHOR_VERIFIED name=\{(.+?)\} origin_dbu=\{(\d+) (\d+)\} "
        r"orient=\{(\S+)\} status=\{(\S+)\}$",
        text,
        re.MULTILINE,
    )
    legality = last(r"^WBQ_B7_PLACE_LEGALITY violations=\{(.*)\}$", text)
    exit_code = last(r"^WBQ_B7_PLACE_EXIT_CODE=(\d+)$", text)
    ...
    checks = {
        ...
        "rudy_complete": "WBQ_B7_RUDY_GLOBAL_PLACEMENT PASS" in text,
        "post_rudy_checkpoint": "WBQ_B7_RUDY_CHECKPOINT PASS" in text,
        "post_legalize_checkpoint": "WBQ_B7_POST_LEGALIZE_CHECKPOINT PASS" in text,
        "lower_max_phi_runtime_marker": "WBQ_B7_RUDY_PLACE_POLICY skip_initial=1 density=0.49 estimator=RUDY min_phi=0.95 max_phi=1.01" in text,
        "failed_gpl0307_absent": "GPL-0307" not in text,
        "final_place_token": "WBQ_B7_LOWER_MAX_PHI_PLACE PASS" in text,
    ...
    payload = {
        ...
        "authorizes": ["B7_TARGETED_PLACEMENT_REOPEN_AUDIT"] if passed else [],
    ...
    with args.output.open("x", encoding="utf-8") as stream:
        ...
        stream.write("\n")
    print(f"WBQ_B7_PLACEMENT_NUMERIC_ANALYSIS {payload['status']} iterations={len(iteration_ids)}")
