#!/usr/bin/env python3
"""Issue a hash-pinned authorization for B19's sole global-route attempt."""
...
PHYSICAL_AUTH = PHYSICAL / "b19_route_pin_smoke_execution_report.json"
...
OUTPUT = REPORT / "b19_global_route_authorization.json"
...
def main() -> int:
    if OUTPUT.exists():
        raise FileExistsError(f"refusing to overwrite B19 route authorization: {OUTPUT}")
    ...
    route_outputs = [
        PHYSICAL / "b19_global_route_invocation.json",
        PHYSICAL / "b19_global_route_execution_report.json",
        PHYSICAL / "b19_global_route.log",
        REPORT / "b19_residual_congestion_analysis.json",
        REPORT / "b19_residual_congestion_analysis.md",
    ]
    artifact_root = Path("/dev/shm/wbq_b19_phase6_10/quad_local_b19")
    route_outputs.extend(
        artifact_root / name
        for name in (
            "b19_quad_local.route_guide",
            "b19_quad_local.congestion.rpt",
            "b19_quad_local_global_route.odb",
            "b19_quad_local_global_route.sdc",
        )
    )
    conditions = {
        ...
        and physical_auth.get("authorizes") == ["B19_ROUTE_AUTHORIZATION"],
        "selected_b19_policy": decision.get("decision") == "SELECT_B19_BUMP_PITCH_ECO"
        and decision.get("selected_eco", {}).get("single_independent_variable") == "route_bump_pin_pitch"
        and decision.get("selected_eco", {}).get("before") == 350.0
        and decision.get("selected_eco", {}).get("after") == 340.0,
        ...
        "b19_only_paths": "quad_local_b7" not in runner_text and "WBQ_B7" not in tcl_text,
    ...
    payload = {
        ...
        "inputs": {
            ...
            "b19_eco_decision": evidence(DECISION),
            "b19_physical_authorization": evidence(PHYSICAL_AUTH),
        ...
        "policy": {
            ...
            "service": "wbq-b19-global-route.service",
            "fail_closed": True,
        },
        "authorizes": ["B19_SINGLE_GLOBAL_ROUTE"] if passed else [],
        "next_stage": "B19_SINGLE_GLOBAL_ROUTE" if passed else None,
    }
    OUTPUT.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    print(f"B19_GLOBAL_ROUTE_AUTHORIZATION {payload['decision']}")
