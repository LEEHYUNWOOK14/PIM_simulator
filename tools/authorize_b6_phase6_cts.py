#!/usr/bin/env python3
"""Hash-pin B6 CTS only after the strict zero-congestion gate passes."""
...
ROUTE = REPORT / "physical/b6_global_route_execution_report.json"
...
OUTPUT = PHASE6 / "b6_phase6_cts_authorization.json"
...
def main() -> int:
    if OUTPUT.exists():
        raise FileExistsError(f"refusing to overwrite B6 CTS authorization: {OUTPUT}")
    gate = load(GATE)
    ...
    phase_root = Path("/dev/shm/wbq_b6_phase6_10/phase6_cts")
    work_result = Path("/dev/shm/wbq_b6_phase6_10/orfs/results/sky130hd/normalization_hbm_quad_local_b6/base")
    prior_outputs = [
        PHASE6 / name
        for name in (
            "b6_phase6_cts_preflight.json", "b6_phase6_cts_invocation.json",
            "b6_phase6_cts_execution_report.json", "b6_phase6_cts.log", "b6_phase6_cts_audit.log",
        )
    ] + [
        phase_root / "b6_phase6_cts.odb", phase_root / "b6_phase6_cts.sdc",
        work_result / "4_1_cts.odb", work_result / "4_cts.odb", work_result / "4_cts.sdc",
    ]
    conditions = {
        "strict_gate_pass": gate.get("decision") == "PASS" and "B6_PHASE6_CTS" in gate.get("authorizes", []),
        ...
        "b6_design_nickname": "DESIGN_NICKNAME = normalization_hbm_quad_local_b6" in config_text,
        "b6_top_name": "DESIGN_NAME = logic_die_normalization_hbm_quad_local_b2_top" in config_text,
    ...
    payload = {
        ...
        "policy": {
            "service": "wbq-b6-phase6-cts.service",
        ...
        "authorizes": ["B6_PHASE6_CTS_COMPUTE"] if passed else [],
        "next_stage": "B6_PHASE6_CTS_COMPUTE" if passed else None,
    }
    PHASE6.mkdir(parents=True, exist_ok=True)
    OUTPUT.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    print(f"B6_PHASE6_CTS_AUTHORIZATION {payload['decision']}")
