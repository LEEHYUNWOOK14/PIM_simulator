#!/usr/bin/env python3
"""Issue a hash-pinned authorization for B15's sole global-route attempt."""
...
PHYSICAL_AUTH = PHYSICAL / "b15_route_pin_smoke_execution_report.json"
...
OUTPUT = REPORT / "b15_global_route_authorization.json"
...
def main() -> int:
    if OUTPUT.exists():
        raise FileExistsError(f"refusing to overwrite B15 route authorization: {OUTPUT}")
    ...
    route_outputs = [
        PHYSICAL / "b15_global_route_invocation.json",
        PHYSICAL / "b15_global_route_execution_report.json",
        PHYSICAL / "b15_global_route.log",
        REPORT / "b15_residual_congestion_analysis.json",
        REPORT / "b15_residual_congestion_analysis.md",
    ]
    artifact_root = Path("/dev/shm/wbq_b15_phase6_10/quad_local_b15")
    route_outputs.extend(
        artifact_root / name
        for name in (
            "b15_quad_local.route_guide",
            "b15_quad_local.congestion.rpt",
            "b15_quad_local_global_route.odb",
            "b15_quad_local_global_route.sdc",
        )
    )
    conditions = {
        ...
        and physical_auth.get("authorizes") == ["B15_ROUTE_AUTHORIZATION"],
        "selected_b15_policy": decision.get("decision") == "SELECT_B15_CLOCK_LAYER_RESTORE_ECO"
        and decision.get("selected_eco", {}).get("single_independent_variable") == "route_clock_layer_bottom"
        ...
        "b15_only_paths": "quad_local_b7" not in runner_text and "WBQ_B7" not in tcl_text,
    ...
    payload = {
        ...
        "inputs": {
            ...
            "b15_eco_decision": evidence(DECISION),
            "b15_physical_authorization": evidence(PHYSICAL_AUTH),
        ...
        "policy": {
            ...
            "service": "wbq-b15-global-route.service",
            "fail_closed": True,
        },
        "authorizes": ["B15_SINGLE_GLOBAL_ROUTE"] if passed else [],
        "next_stage": "B15_SINGLE_GLOBAL_ROUTE" if passed else None,
    }
    OUTPUT.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    print(f"B15_GLOBAL_ROUTE_AUTHORIZATION {payload['decision']}")
