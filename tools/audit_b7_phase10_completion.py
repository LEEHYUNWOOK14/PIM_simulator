#!/usr/bin/env python3
"""Fail-closed Phase 10 audit for the complete B7 research-artifact chain."""
...
PHASE9_ROOT = Path(
    os.environ.get("WBQ_B7_PHASE9_ROOT", str(ROOT / "output/final_integrated_gds/b7"))
)
EVIDENCE = B7 / "phase10"
PREFLIGHT = EVIDENCE / "b7_phase10_preflight.json"
HASH_MANIFEST = EVIDENCE / "b7_phase6_phase10_artifact_hash_manifest.json"
COMPLETION = EVIDENCE / "b7_phase10_completion_report.json"
MARKDOWN = EVIDENCE / "b7_phase10_completion_report.md"
EVIDENCE_MATRIX = EVIDENCE / "b7_phase10_evidence_matrix.json"
...
CHAIN = {
    ...
    # B3/B4/B5/B6/B7 variant sequence. B7 reuses the sealed B5 functional gate
    # because its ECO changes only one physical optimizer coefficient. B6 is
    # retained as an explicit sealed GPL-0307 pre-route failure.
    "root_cause": ROOT / "reports/groot_normalization/quad_local_b3/b3_cheap_root_cause_analysis.json",
    "sealed_b6_failure": ROOT / "reports/groot_normalization/quad_local_b6/physical/b6_placement_execution_report.json",
    "eco_decision": B7 / "b7_eco_decision.json",
    "sealed_b5_cheap_gate": ROOT / "reports/groot_normalization/quad_local_b5/cheap_gate_manifest.json",
    "b7_smoke": B7 / "physical/b7_smoke_execution_report.json",
    "physical_authorization": B7 / "b7_physical_authorization.json",
    "placement": B7 / "physical/b7_placement_execution_report.json",
    "placement_numeric_analysis": B7 / "physical/b7_placement_numeric_analysis.json",
    "targeted_placement_audit": B7 / "physical/b7_targeted_placement_reopen_audit.json",
    "global_route_authorization": B7 / "b7_global_route_authorization.json",
    "global_route": B7 / "physical/b7_global_route_execution_report.json",
    "congestion_analysis": B7 / "b7_residual_congestion_analysis.json",
    "phase6_gate": B7 / "phase6_decision_gate.json",
    "phase6_cts_authorization": B7 / "phase6/b7_phase6_cts_authorization.json",
    "phase6_cts": B7 / "phase6/b7_phase6_cts_execution_report.json",
    "phase6_post_cts_authorization": B7 / "phase6/b7_phase6_post_cts_route_authorization.json",
    "phase6_post_cts": B7 / "phase6/b7_phase6_post_cts_global_route_execution_report.json",
    "phase7_authorization": B7 / "phase7/b7_phase7_detailed_route_authorization.json",
    "phase7_detailed_route": B7 / "phase7/b7_phase7_detailed_route_execution_report.json",
    "phase7_rtl_gds": B7 / "phase7/b7_phase7_rtl_gds_execution_report.json",
    "phase8_overlay": B7 / "phase8/b7_phase8_overlay_execution_report.json",
    "phase9_final_gds": B7 / "phase9/b7_phase9_final_gds_execution_report.json",
}
...
def main() -> int:
    ...
    if phase9.get("status") != "PASS" or "B7_PHASE10_COMPLETION_AUDIT" not in phase9.get("authorizes", []):
        raise RuntimeError("Phase 9 does not authorize Phase 10")

    generated = datetime.now(timezone.utc).isoformat()
    with PREFLIGHT.open("x", encoding="utf-8") as stream:
        json.dump({
            "schema_version": 1, "phase": 10, "variant": "B7_PHASE10",
            "status": "PASS", "generated_at_utc": generated,
            "phase9_input": artifact(CHAIN["phase9_final_gds"]),
            "openroad_processes_before": [],
            "authorizes": ["B7_PHASE10_COMPLETION_AUDIT_COMPUTE"],
            "next_stage": "B7_PHASE10_COMPLETION_AUDIT_COMPUTE",
        }, stream, indent=2)
        stream.write("\n")
    ...
    phase_checks = {
        ...
        "sealed_b6_gpl0307_pre_route_failure": docs["sealed_b6_failure"].get("status") == "FAIL"
        and docs["sealed_b6_failure"].get("exit_code") == 1
        and docs["sealed_b6_failure"].get("global_route_invocations") == 0
        and docs["sealed_b6_failure"].get("protected_artifacts_preserved") is True
        and docs["sealed_b6_failure"].get("authorizes") == [],
        "b7_one_variable_phi_decision": docs["eco_decision"].get("decision") == "SELECT_B7_LOWER_MAX_PHI_ECO"
        and docs["eco_decision"].get("selected_eco", {}).get("before") == 1.05
        and docs["eco_decision"].get("selected_eco", {}).get("after") == 1.01
        and docs["eco_decision"].get("failure_classification", {}).get("subtype")
        == "GPL-0307 optimizer_numerical_divergence",
        "b7_fresh_smoke": docs["b7_smoke"].get("status") == "PASS"
        and docs["b7_smoke"].get("protected_artifacts_preserved") is True,
        ...
        and docs["phase6_gate"].get("authorizes") == ["B7_PHASE6_CTS"],
    ...
    phase_checks["b7_zero_congestion"] = congestion.get("rrr_residual") == 0 and congestion.get("overflow_edges") == 0
    ...
    artifact_checks = {}
    for name, key in (("b7_smoke", "outputs"), ("placement", "outputs"), ("targeted_placement_audit", "outputs"),
    ...
    clean_parent = Path("/dev/shm/wbq_b7_phase6_10")
    clean_parent.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(prefix="b7_phase10_clean_", dir=clean_parent) as temp_name:
        temp = Path(temp_name)
    ...
    with tempfile.TemporaryDirectory(prefix="b7_phase10_klayout_", dir=clean_parent) as temp_name:
    ...
    hash_payload = {
        ...
        "variant": "B7_PHASE6_PHASE10", "artifacts": [artifact(path) for path in unique],
    ...
    completion = {
        "schema_version": 1, "generated_at_utc": datetime.now(timezone.utc).isoformat(),
        "phase": 10, "variant": "B7", "status": "PASS" if all_pass else "FAIL",
        ...
        "metrics": {"b7_rrr_residual": congestion.get("rrr_residual"),
                    "b7_overflow_edges": congestion.get("overflow_edges"),
        ...
        "known_limitations": {"manufacturing_signoff": False, "timing_signoff": False,
                              ...
                              "overlay_geometry": "illustrative/estimated",
                              "intermediate_large_artifact_storage": "/dev/shm/wbq_b7_phase6_10",
    ...
    MARKDOWN.write_text(
        ...
        f"- B7 residual/overflow: `{congestion.get('rrr_residual')}/{congestion.get('overflow_edges')}`\n"
    ...
    HTML.write_text(
        ...
        "<title>B7 Phase 10 completion</title>"
        ...
        "</head><body><h1>B7 Phase 10 completion report</h1>"
    ...
    print(f"WBQ_B7_PHASE10_COMPLETION {completion['status']} gds_sha256={original_hash}")
