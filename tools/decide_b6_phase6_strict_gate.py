#!/usr/bin/env python3
"""Evaluate B6's four fail-closed Phase 6 release conditions."""
...
ROUTE_MANIFEST = PHYSICAL / "b6_global_route_execution_report.json"
ANALYSIS = REPORT / "b6_residual_congestion_analysis.json"
...
def main() -> int:
    ...
    payload = {
        ...
        "reason": (
            "B6 has zero RRR residual, zero numeric overflow edges, matching artifact hashes, and an explicit Phase 6 PASS."
            if passed
        ...
        "authorizes": ["B6_PHASE6_CTS"] if passed else [],
        "next_stage": "B6_PHASE6_CTS" if passed else None,
        "blocked_recovery": None if passed else "Seal B6 without reroute; select one evidence-based B7 ECO and re-run smoke plus authorization.",
    }
    OUTPUT.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    print(
        f"B6_PHASE6_STRICT_GATE {payload['decision']} residual={residual} "
