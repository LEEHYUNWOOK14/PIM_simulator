#!/usr/bin/env python3
"""Independently reopen and gate the B6 Phase 7 routed RTL GDS."""
...
def main() -> int:
    ...
    parser.add_argument("--cell-gds", type=Path, required=True)
    parser.add_argument("--converter", type=Path, required=True)
    parser.add_argument("--output", type=Path, required=True)
    args = parser.parse_args()
    if args.output.exists():
        raise FileExistsError(f"refusing to overwrite {args.output}")
    for path in (args.gds, args.source, args.log, args.technology, args.cell_gds, args.converter):
        if not path.is_file() or path.stat().st_size == 0:
            raise FileNotFoundError(path)
    source = json.loads(args.source.read_text(encoding="utf-8"))
    log = args.log.read_text(encoding="utf-8", errors="replace")
    actual_hash = sha256(args.gds)
    logged = re.findall(r"^WBQ_B6_RTL_GDS_OUTPUT_SHA256=(\S+)$", log, re.MULTILINE)
    layout = kdb.Layout()
    ...
    checks = {
        ...
        and "B6_PHASE7_RTL_GDS_STREAMOUT" in source.get("authorizes", []),
        "streamout_exit_zero": "WBQ_B6_RTL_GDS_EXIT_CODE=0" in log,
    ...
    payload = {
        "schema_version": 1, "phase": 7, "variant": "B6_PHASE7_RTL_GDS",
        ...
        "inputs": {
            "phase7_manifest": {"path": str(args.source), "sha256": sha256(args.source)},
            "streamout_log": {"path": str(args.log), "sha256": sha256(args.log)},
            "technology": {"path": str(args.technology), "sha256": sha256(args.technology)},
            "cell_gds": {"path": str(args.cell_gds), "sha256": sha256(args.cell_gds)},
        ...
        "authorizes": ["B6_PHASE8_OVERLAY"] if passed else [],
        "next_stage": "B6_PHASE8_OVERLAY" if passed else None,
    ...
    print(f"WBQ_B6_PHASE7_RTL_GDS_READBACK {payload['status']} sha256={actual_hash}")
