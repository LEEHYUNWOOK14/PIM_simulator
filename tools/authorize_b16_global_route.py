#!/usr/bin/env python3
"""Issue a hash-pinned authorization for B16's sole global-route attempt."""
...
PHYSICAL_AUTH = PHYSICAL / "b16_route_pin_smoke_execution_report.json"
...
OUTPUT = REPORT / "b16_global_route_authorization.json"
...
def main() -> int:
    if OUTPUT.exists():
        raise FileExistsError(f"refusing to overwrite B16 route authorization: {OUTPUT}")
    ...
    route_outputs = [
        PHYSICAL / "b16_global_route_invocation.json",
        PHYSICAL / "b16_global_route_execution_report.json",
        PHYSICAL / "b16_global_route.log",
        REPORT / "b16_residual_congestion_analysis.json",
        REPORT / "b16_residual_congestion_analysis.md",
    ]
    artifact_root = Path("/dev/shm/wbq_b16_phase6_10/quad_local_b16")
    route_outputs.extend(
        artifact_root / name
        for name in (
            "b16_quad_local.route_guide",
            "b16_quad_local.congestion.rpt",
            "b16_quad_local_global_route.odb",
            "b16_quad_local_global_route.sdc",
        )
    )
    conditions = {
        ...
        and physical_auth.get("authorizes") == ["B16_ROUTE_AUTHORIZATION"],
        "selected_b16_policy": decision.get("decision") == "SELECT_B16_BUMP_GRID_COLUMNS_ECO"
        and decision.get("selected_eco", {}).get("single_independent_variable") == "route_bump_pin_grid_columns"
        and decision.get("selected_eco", {}).get("before") == 17
        and decision.get("selected_eco", {}).get("after") == 23,
        ...
        "b16_only_paths": "quad_local_b7" not in runner_text and "WBQ_B7" not in tcl_text,
    ...
    payload = {
        ...
        "inputs": {
            ...
            "b16_eco_decision": evidence(DECISION),
            "b16_physical_authorization": evidence(PHYSICAL_AUTH),
        ...
        "policy": {
            ...
            "service": "wbq-b16-global-route.service",
            "fail_closed": True,
        },
        "authorizes": ["B16_SINGLE_GLOBAL_ROUTE"] if passed else [],
        "next_stage": "B16_SINGLE_GLOBAL_ROUTE" if passed else None,
    }
    OUTPUT.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    print(f"B16_GLOBAL_ROUTE_AUTHORIZATION {payload['decision']}")
