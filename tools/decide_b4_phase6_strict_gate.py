#!/usr/bin/env python3
"""Evaluate the four fail-closed Phase 6 release conditions for B4."""
...
ROUTE_MANIFEST = PHYSICAL / "b4_global_route_execution_report.json"
ANALYSIS = REPORT / "b4_residual_congestion_analysis.json"
...
def main() -> int:
    ...
    payload = {
        ...
        "reason": (
            "B4 has zero RRR residual, zero numeric overflow edges, matching input hashes, and this explicit PASS."
            if passed else
        ...
        "authorizes": ["B4_PHASE6_CTS"] if passed else [],
        "next_stage": "B4_PHASE6_CTS" if passed else None,
        "blocked_recovery": None if passed else "Seal B4 without reroute; create a separately justified B5 ECO variant.",
    }
    with OUTPUT.open("x", encoding="utf-8") as stream:
        json.dump(payload, stream, indent=2)
        stream.write("\n")
    print(
        f"B4_PHASE6_STRICT_GATE {payload['decision']} residual={residual} "
