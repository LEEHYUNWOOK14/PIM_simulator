#!/usr/bin/env python3
"""Issue a hash-pinned authorization for B20's sole global-route attempt."""
...
PHYSICAL_AUTH = PHYSICAL / "b20_route_pin_smoke_execution_report.json"
...
OUTPUT = REPORT / "b20_global_route_authorization.json"
...
def main() -> int:
    if OUTPUT.exists():
        raise FileExistsError(f"refusing to overwrite B20 route authorization: {OUTPUT}")
    ...
    route_outputs = [
        PHYSICAL / "b20_global_route_invocation.json",
        PHYSICAL / "b20_global_route_execution_report.json",
        PHYSICAL / "b20_global_route.log",
        REPORT / "b20_residual_congestion_analysis.json",
        REPORT / "b20_residual_congestion_analysis.md",
    ]
    artifact_root = Path("/dev/shm/wbq_b20_phase6_10/quad_local_b20")
    route_outputs.extend(
        artifact_root / name
        for name in (
            "b20_quad_local.route_guide",
            "b20_quad_local.congestion.rpt",
            "b20_quad_local_global_route.odb",
            "b20_quad_local_global_route.sdc",
        )
    )
    conditions = {
        ...
        and physical_auth.get("authorizes") == ["B20_ROUTE_AUTHORIZATION"],
        "selected_b20_policy": decision.get("decision") == "SELECT_B20_BUMP_Y_ORIGIN_ECO"
        and decision.get("selected_eco", {}).get("single_independent_variable") == "route_bump_pin_y_origin"
        and decision.get("selected_eco", {}).get("before") == 1500.0
        and decision.get("selected_eco", {}).get("after") == 1000.0,
        ...
        "b20_only_paths": "quad_local_b7" not in runner_text and "WBQ_B7" not in tcl_text,
    ...
    payload = {
        ...
        "inputs": {
            ...
            "b20_eco_decision": evidence(DECISION),
            "b20_physical_authorization": evidence(PHYSICAL_AUTH),
        ...
        "policy": {
            ...
            "service": "wbq-b20-global-route.service",
            "fail_closed": True,
        },
        "authorizes": ["B20_SINGLE_GLOBAL_ROUTE"] if passed else [],
        "next_stage": "B20_SINGLE_GLOBAL_ROUTE" if passed else None,
    }
    OUTPUT.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    print(f"B20_GLOBAL_ROUTE_AUTHORIZATION {payload['decision']}")
