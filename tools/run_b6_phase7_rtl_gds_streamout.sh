...
evidence="$root/reports/groot_normalization/quad_local_b6/phase7"
...
def="$phase7_root/b6_phase7_detailed_route.def"
...
gds="$phase7_root/b6_phase7_routed.gds"
log="$evidence/b6_phase7_rtl_gds_streamout.log"
preflight="$evidence/b6_phase7_rtl_gds_preflight.json"
attempt="$evidence/b6_phase7_rtl_gds_invocation.json"
output_manifest="$evidence/b6_phase7_rtl_gds_execution_report.json"
...
if manifest.get("status") != "PASS" or "B6_PHASE7_RTL_GDS_STREAMOUT" not in manifest.get("authorizes", []):
    raise SystemExit("B6 Phase 7 manifest does not authorize GDS streamout")
if sha(pathlib.Path(sys.argv[2])) != manifest["artifacts"]["detailed_def"]["sha256"]:
    raise SystemExit("B6 detailed DEF hash mismatch")
print("WBQ_B6_PHASE7_RTL_GDS_INPUT_GATE PASS")
PY
for path in "$gds" "$log" "$preflight" "$attempt" "$output_manifest"; do
  if [[ -e "$path" ]]; then echo "existing B6 RTL GDS artifact prevents overwrite: $path" >&2; exit 4; fi
done
...
payload = {"schema_version": 1, "phase": 7, "variant": "B6_PHASE7_RTL_GDS",
           ...
           "authorizes": ["B6_PHASE7_RTL_GDS_STREAMOUT_COMPUTE"],
           "next_stage": "B6_PHASE7_RTL_GDS_STREAMOUT_COMPUTE"}
...
{
  echo "WBQ_B6_RTL_GDS_START_UTC=$start"
  echo "WBQ_B6_RTL_GDS_INVOCATION_COUNT=1"
  echo "WBQ_B6_RTL_GDS_MANIFEST_SHA256=$manifest_sha"
  echo "WBQ_B6_RTL_GDS_DEF_SHA256=$def_sha"
  echo "WBQ_B6_RTL_GDS_STREAMOUT_METHOD=ORFS_KLAYOUT_DEF2STREAM_FROM_OPENROAD_DETAILED_DEF"
  echo "WBQ_B6_RTL_GDS_KLAYOUT_VERSION=$(klayout -b -v 2>&1 | head -n 1)"
  echo "WBQ_B6_RTL_GDS_TECH_SHA256=$tech_sha"
  echo "WBQ_B6_RTL_GDS_CELL_LIBRARY_SHA256=$cell_gds_sha"
  echo "WBQ_B6_RTL_GDS_CONVERTER_SHA256=$converter_sha"
} > "$log"
set +e
...
rc=$?
set -e
echo "WBQ_B6_RTL_GDS_EXIT_CODE=$rc" >> "$log"
echo "WBQ_B6_RTL_GDS_END_UTC=$(date -u +%Y-%m-%dT%H:%M:%SZ)" >> "$log"
if [[ "$rc" -eq 0 && -s "$gds" ]]; then
  echo "WBQ_B6_RTL_GDS_OUTPUT_SHA256=$(sha256sum "$gds" | awk '{print $1}')" >> "$log"
fi
test "$rc" -eq 0
test -s "$gds"
python3 tools/check_b6_phase7_rtl_gds.py --gds "$gds" --source "$manifest" --log "$log" \
  --technology "$tech" --cell-gds "$cell_gds" --converter "$converter" --output "$output_manifest"
grep -q '"status": "PASS"' "$output_manifest"
echo "WBQ_B6_PHASE7_RTL_GDS_STREAMOUT PASS report=$output_manifest"
