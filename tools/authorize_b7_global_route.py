#!/usr/bin/env python3
"""Issue a hash-pinned authorization for B7's sole global-route attempt."""
...
NUMERIC_ANALYSIS = PHYSICAL / "b7_placement_numeric_analysis.json"
...
OUTPUT = REPORT / "b7_global_route_authorization.json"
...
def main() -> int:
    if OUTPUT.exists():
        raise FileExistsError(f"refusing to overwrite B7 route authorization: {OUTPUT}")
    ...
    route_outputs = [
        PHYSICAL / "b7_global_route_invocation.json",
        PHYSICAL / "b7_global_route_execution_report.json",
        PHYSICAL / "b7_global_route.log",
        REPORT / "b7_residual_congestion_analysis.json",
        REPORT / "b7_residual_congestion_analysis.md",
    ]
    artifact_root = Path("/dev/shm/wbq_b7_phase6_10/quad_local_b7")
    route_outputs.extend(
        artifact_root / name
        for name in (
            "b7_quad_local.route_guide",
            "b7_quad_local.congestion.rpt",
            "b7_quad_local_global_route.odb",
            "b7_quad_local_global_route.sdc",
        )
    )
    conditions = {
        "placement_pass": placement.get("status") == "PASS",
        "placement_audit_pass": placement.get("placement_legality_and_fence_audit") == "PASS",
        "placement_authorizes_numeric_and_reopen": "B7_TARGETED_PLACEMENT_REOPEN_AUDIT" in placement.get("authorizes", []),
        "placement_outputs_match": placement_outputs_match,
        "placement_numeric_analysis_pass": numeric.get("status") == "PASS"
        and numeric.get("authorizes") == ["B7_TARGETED_PLACEMENT_REOPEN_AUDIT"],
        ...
        and "B7_GLOBAL_ROUTE_AUTHORIZATION" in targeted_audit.get("authorizes", []),
        ...
        "selected_b7_policy": decision.get("decision") == "SELECT_B7_LOWER_MAX_PHI_ECO",
        ...
        "b7_only_paths": "quad_local_b6" not in runner_text and "WBQ_B6" not in tcl_text,
    ...
    payload = {
        ...
        "inputs": {
            ...
            "b7_eco_decision": evidence(DECISION),
            "b7_physical_authorization": evidence(PHYSICAL_AUTH),
        ...
        "policy": {
            ...
            "service": "wbq-b7-global-route.service",
            "fail_closed": True,
        },
        "authorizes": ["B7_SINGLE_GLOBAL_ROUTE"] if passed else [],
        "next_stage": "B7_SINGLE_GLOBAL_ROUTE" if passed else None,
    }
    OUTPUT.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    print(f"B7_GLOBAL_ROUTE_AUTHORIZATION {payload['decision']}")
