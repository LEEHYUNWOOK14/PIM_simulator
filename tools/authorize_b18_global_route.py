#!/usr/bin/env python3
"""Issue a hash-pinned authorization for B18's sole global-route attempt."""
...
PHYSICAL_AUTH = PHYSICAL / "b18_route_pin_smoke_execution_report.json"
...
OUTPUT = REPORT / "b18_global_route_authorization.json"
...
def main() -> int:
    if OUTPUT.exists():
        raise FileExistsError(f"refusing to overwrite B18 route authorization: {OUTPUT}")
    ...
    route_outputs = [
        PHYSICAL / "b18_global_route_invocation.json",
        PHYSICAL / "b18_global_route_execution_report.json",
        PHYSICAL / "b18_global_route.log",
        REPORT / "b18_residual_congestion_analysis.json",
        REPORT / "b18_residual_congestion_analysis.md",
    ]
    artifact_root = Path("/dev/shm/wbq_b18_phase6_10/quad_local_b18")
    route_outputs.extend(
        artifact_root / name
        for name in (
            "b18_quad_local.route_guide",
            "b18_quad_local.congestion.rpt",
            "b18_quad_local_global_route.odb",
            "b18_quad_local_global_route.sdc",
        )
    )
    conditions = {
        ...
        and physical_auth.get("authorizes") == ["B18_ROUTE_AUTHORIZATION"],
        "selected_b18_policy": decision.get("decision") == "SELECT_B18_BUMP_PITCH_ECO"
        and decision.get("selected_eco", {}).get("single_independent_variable") == "route_bump_pin_pitch"
        and decision.get("selected_eco", {}).get("before") == 300.0
        and decision.get("selected_eco", {}).get("after") == 350.0,
        ...
        "b18_only_paths": "quad_local_b7" not in runner_text and "WBQ_B7" not in tcl_text,
    ...
    payload = {
        ...
        "inputs": {
            ...
            "b18_eco_decision": evidence(DECISION),
            "b18_physical_authorization": evidence(PHYSICAL_AUTH),
        ...
        "policy": {
            ...
            "service": "wbq-b18-global-route.service",
            "fail_closed": True,
        },
        "authorizes": ["B18_SINGLE_GLOBAL_ROUTE"] if passed else [],
        "next_stage": "B18_SINGLE_GLOBAL_ROUTE" if passed else None,
    }
    OUTPUT.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
    print(f"B18_GLOBAL_ROUTE_AUTHORIZATION {payload['decision']}")
