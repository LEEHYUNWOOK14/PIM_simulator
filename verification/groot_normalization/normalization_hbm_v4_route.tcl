...
# Model command/read HBM boundary signals as distributed logic-die landing
# pads instead of forcing every wide boundary bit through the die edge.
set bump_terms {}
...
puts "V4_BUMP_PIN_COUNT [llength $bump_terms]"

# Use the platform's complete signal-layer range.  One CUGR iteration keeps
# the full-net feasibility attempt bounded while still producing a real guide
# and detailed congestion report.
set_routing_layers -signal met1-met5 -clock met2-met5
puts "V4_ROUTE_STAGE global_route"
global_route \
  -guide_file $report_root/logic_die_normalization_hbm_top_v4.route_guide \
  -congestion_report_file $report_root/logic_die_normalization_hbm_top_v4.congestion.rpt \
