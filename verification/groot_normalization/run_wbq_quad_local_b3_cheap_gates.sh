...
result="$root/reports/groot_normalization/results/quad_local_b3_actual_trace"
...
if data.get("decision") != "SELECT_B3_ROUTE_EFFORT_ECO":
    raise SystemExit("B3 ECO decision is not selected")
if data.get("authorizes") != []:
    raise SystemExit("pre-cheap B3 ECO decision must remain fail-closed")
print("B3_ECO_INPUT_GATE PASS")
PY
...
if pgrep -x openroad >/dev/null; then
  echo "OpenROAD process exists before B3 read-only floorplan audit" >&2
  exit 3
fi
{
  echo "WBQ_B3_FLOORPLAN_AUDIT_MODE=READ_ONLY_B2_SAME_NETLIST_AND_GEOMETRY"
  echo "WBQ_B3_FLOORPLAN_ODB_SHA256=$(sha256sum "$b2_floorplan" | awk '{print $1}')"
  echo "WBQ_B3_FLOORPLAN_SDC_SHA256=$(sha256sum "$b2_floorplan_sdc" | awk '{print $1}')"
} > "$report/b3_floorplan_preflight.log"
WBQ_QUAD_FENCE_ODB="$b2_floorplan" WBQ_QUAD_FENCE_DENSITY=0.39 \
  ...
  >> "$report/b3_floorplan_preflight.log" 2>&1
grep -q "WBQ_QUAD_FENCE_CAPACITY PASS" "$report/b3_floorplan_preflight.log"

WBQ_B2=1 WBQ_VARIANT=B3 python3 tools/collect_wbq_quad_local_cheap_gate.py
...
if data.get("authorizes") != ["B3_PLACEMENT", "B3_SINGLE_GLOBAL_ROUTE"]:
    raise SystemExit("B3 cheap authorization mismatch")
print("WBQ_B3_CHEAP_GATES PASS gates=9")
PY
