...
artifact_root="${WBQ_B6_ARTIFACT_ROOT:-/dev/shm/wbq_b6_phase6_10/quad_local_b6}"
phase_root="${WBQ_B6_PHASE6_ROOT:-/dev/shm/wbq_b6_phase6_10/phase6_cts}"
work_home="${WBQ_B6_ORFS_WORK_HOME:-/dev/shm/wbq_b6_phase6_10/orfs}"
orfs_result="$work_home/results/sky130hd/normalization_hbm_quad_local_b6/base"
config="$root/flow/designs/sky130hd/normalization_hbm_quad_local_b6/config.mk"
gate="$report/phase6_decision_gate.json"
authorization="$evidence/b6_phase6_cts_authorization.json"
placement="$report/physical/b6_placement_execution_report.json"
place_odb="$artifact_root/b6_place.odb"
place_sdc="$artifact_root/b6_place.sdc"
cts_odb="$phase_root/b6_phase6_cts.odb"
cts_sdc="$phase_root/b6_phase6_cts.sdc"
log="$evidence/b6_phase6_cts.log"
audit_log="$evidence/b6_phase6_cts_audit.log"
preflight="$evidence/b6_phase6_cts_preflight.json"
attempt="$evidence/b6_phase6_cts_invocation.json"
manifest="$evidence/b6_phase6_cts_execution_report.json"
runner="$root/verification/groot_normalization/run_wbq_b6_phase6_cts.sh"
audit_tcl="$root/verification/groot_normalization/audit_wbq_b6_phase6_cts.tcl"
snapshot_tool="$root/tools/capture_openroad_stage_snapshot.py"
compare_tool="$root/tools/compare_openroad_stage_snapshots.py"
runner_service="${WBQ_RUNNER_SERVICE:-wbq-b6-phase6-cts.service}"
...
if gate.get("decision") != "PASS" or "B6_PHASE6_CTS" not in gate.get("authorizes", []):
    raise SystemExit("strict Phase 6 gate does not authorize B6 CTS")
if authorization.get("decision") != "PASS" or "B6_PHASE6_CTS_COMPUTE" not in authorization.get("authorizes", []):
    raise SystemExit("fresh B6 CTS authorization is not PASS")
if placement.get("status") != "PASS":
    raise SystemExit("B6 placement is not PASS")
for key, text in (("b6_place_odb", sys.argv[4]), ("b6_place_sdc", sys.argv[5])):
    path = pathlib.Path(text)
    if sha(path) != placement["outputs"][key]["sha256"]:
        raise SystemExit(f"B6 CTS placement input hash mismatch: {key}")
for key, text in (
    ...
    if sha(path) != authorization["inputs"][key]["sha256"]:
        raise SystemExit(f"B6 CTS authorization hash mismatch: {key}")
print("WBQ_B6_PHASE6_CTS_INPUT_GATE PASS")
PY
...
for path in "$preflight" "$attempt" "$manifest" "$log" "$audit_log" "$cts_odb" "$cts_sdc" \
  ...
  if [[ -e "$path" ]]; then
    echo "existing B6 Phase 6 CTS artifact prevents a fresh invocation: $path" >&2
    exit 4
  fi
done
if pgrep -x openroad >/dev/null; then
  echo "an OpenROAD process already exists; refusing concurrent B6 Phase 6 CTS" >&2
...
if (( free_kib < 16 * 1024 * 1024 )); then
  echo "less than 16 GiB free in B6 Phase 6 storage" >&2
  exit 5
fi
if (( available_kib < 32 * 1024 * 1024 )); then
  echo "less than 32 GiB MemAvailable for B6 CTS" >&2
  exit 6
fi
...
payload = {
    ...
    "command": "make -C ORFS WORK_HOME=<B6 tmpfs> DESIGN_CONFIG=<B6 config> -j1 do-4_1_cts",
    ...
    "expected_artifacts": ["b6_phase6_cts.odb", "b6_phase6_cts.sdc"],
    "authorizes": ["B6_PHASE6_CTS_COMPUTE"],
    "next_stage": "B6_PHASE6_CTS_COMPUTE",
}
...
on_signal() {
  ...
  echo "WBQ_B6_PHASE6_CTS_SIGNAL=$termination_signal" >> "$log"
  refresh_compute_pid
...
payload = {
    "schema_version": 2, "phase": 6, "variant": "B6_PHASE6_CTS", "status": "FAIL",
...
{
  echo "WBQ_B6_PHASE6_CTS_START_UTC=$start"
  echo "WBQ_B6_PHASE6_CTS_INVOCATION_COUNT=1"
  echo "WBQ_B6_PHASE6_CTS_SERVICE=$runner_service"
  echo "WBQ_B6_PHASE6_CTS_WRAPPER_PID=$$"
  echo "WBQ_B6_PHASE6_CTS_GATE_SHA256=$gate_sha"
  echo "WBQ_B6_PHASE6_CTS_AUTHORIZATION_SHA256=$authorization_sha"
  echo "WBQ_B6_PHASE6_CTS_PLACEMENT_SHA256=$placement_sha"
  echo "WBQ_B6_PHASE6_CTS_PLACE_ODB_SHA256=$place_odb_sha"
  echo "WBQ_B6_PHASE6_CTS_PLACE_SDC_SHA256=$place_sdc_sha"
  echo "WBQ_B6_PHASE6_CTS_POLICY=ORFS_TRITONCTS_REPAIR_CLOCK_NETS"
  echo "WBQ_B6_PHASE6_CTS_SIGNAL_LAYERS=met1-met5"
  echo "WBQ_B6_PHASE6_CTS_CLOCK_LAYERS=met2-met5"
} > "$log"
...
for _ in $(seq 1 120); do
  ...
  if [[ -n "$compute_pid" ]]; then
    echo "WBQ_B6_PHASE6_CTS_COMPUTE_PID=$compute_pid" >> "$log"
...
set -e
compute_pid=""
echo "WBQ_B6_PHASE6_CTS_EXIT_CODE=$rc" >> "$log"
...
if [[ "$rc" -eq 0 && -s "$cts_odb" && -s "$cts_sdc" ]]; then
  {
    echo "WBQ_B6_PHASE6_CTS_AUDIT_START_UTC=$(date -u +%Y-%m-%dT%H:%M:%SZ)"
    echo "WBQ_B6_PHASE6_CTS_AUDIT_ODB_SHA256=$(sha256sum "$cts_odb" | awk '{print $1}')"
    echo "WBQ_B6_PHASE6_CTS_AUDIT_SDC_SHA256=$(sha256sum "$cts_sdc" | awk '{print $1}')"
  } > "$audit_log"
...
PY
  ...
  set -e
  audit_compute_pid=""
  echo "WBQ_B6_PHASE6_CTS_AUDIT_END_UTC=$(date -u +%Y-%m-%dT%H:%M:%SZ)" >> "$audit_log"
fi
end="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
echo "WBQ_B6_PHASE6_CTS_END_UTC=$end" >> "$log"
...
clock_nets = one(r"^WBQ_B6_PHASE6_CTS_AUDIT_CLOCK_NET_COUNT (\d+)$", audit_text)
attempt_data = json.load(open(attempt, encoding="utf-8"))
passed = (int(rc) == 0 and int(audit_rc) == 0 and not signal
          and "WBQ_B6_PHASE6_CTS_AUDIT PASS" in audit_text
...
payload = {
    "schema_version": 2, "phase": 6, "variant": "B6_PHASE6_CTS",
    ...
    "artifacts": {"cts_odb": item(odb), "cts_sdc": item(sdc), "run_log": item(log), "audit_log": item(audit_log)},
    "authorizes": ["B6_PHASE6_POST_CTS_GLOBAL_ROUTE_AUTHORIZATION"] if passed else [],
    "next_stage": "B6_PHASE6_POST_CTS_GLOBAL_ROUTE_AUTHORIZATION" if passed else None,
...
echo "WBQ_B6_PHASE6_CTS PASS report=$manifest"
