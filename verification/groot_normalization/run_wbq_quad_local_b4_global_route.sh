...
placement="$physical/b4_placement_execution_report.json"
place_odb="$artifacts/b4_place.odb"
place_sdc="$artifacts/b4_place.sdc"
log="$physical/b4_global_route.log"
guide="$artifacts/b4_quad_local.route_guide"
congestion="$artifacts/b4_quad_local.congestion.rpt"
routed_odb="$artifacts/b4_quad_local_global_route.odb"
routed_sdc="$artifacts/b4_quad_local_global_route.sdc"
attempt="$physical/b4_global_route_invocation.json"
manifest="$physical/b4_global_route_execution_report.json"
analysis_json="$report/b4_residual_congestion_analysis.json"
analysis_md="$report/b4_residual_congestion_analysis.md"
tcl="$root/verification/groot_normalization/wbq_quad_local_b4_global_route.tcl"
...
if data.get("status") != "PASS" or data.get("placement_legality_and_fence_audit") != "PASS":
    raise SystemExit("B4 placement gate is not PASS")
if "B4_SINGLE_GLOBAL_ROUTE" not in data.get("authorizes", []):
    raise SystemExit("B4 placement does not authorize the single global route")
for key, text in (("b4_place_odb", sys.argv[2]), ("b4_place_sdc", sys.argv[3])):
    ...
    if digest.hexdigest() != data["outputs"][key]["sha256"]:
        raise SystemExit(f"B4 placement artifact hash mismatch: {key}")
print("WBQ_B4_SINGLE_GLOBAL_ROUTE_AUTHORIZATION PASS")
PY

# Any one of these files proves that B4 consumed its sole route allowance.
for path in "$attempt" "$manifest" "$log" "$guide" "$congestion" "$routed_odb" "$routed_sdc" "$analysis_json" "$analysis_md"; do
  if [[ -e "$path" ]]; then
    echo "B4 global route has already been attempted; refusing duplicate ($path)" >&2
    exit 4
  fi
done
if pgrep -x openroad >/dev/null; then
  echo "an OpenROAD process already exists; refusing concurrent B4 global route" >&2
  pgrep -a -x openroad >&2 || true
  exit 3
fi
free_kib="$(df -Pk "$artifacts" | awk 'NR==2 {print $4}')"
if (( free_kib < 8 * 1024 * 1024 )); then
  echo "less than 8 GiB free; refusing B4 single global route" >&2
  exit 5
fi
...
payload = {
    "schema_version": 1, "variant": "B4", "stage": "global_route",
    "status_at_creation": "STARTED", "invocation_count": 1,
    "cugr_congestion_iterations": 1, "start_utc": start,
    "command": "openroad -exit -no_init -threads 1 -no_splash wbq_quad_local_b4_global_route.tcl",
    "inputs": {
...
{
  echo "WBQ_B4_GLOBAL_ROUTE_START_UTC=$start"
  echo "WBQ_B4_GLOBAL_ROUTE_INVOCATION_COUNT=1"
  echo "WBQ_B4_CUGR_CONGESTION_ITERATIONS=1"
  echo "WBQ_B4_GLOBAL_ROUTE_PLACE_ODB_SHA256=$place_odb_sha"
  echo "WBQ_B4_GLOBAL_ROUTE_PLACE_SDC_SHA256=$place_sdc_sha"
} > "$log"
export WBQ_PLATFORM_ROOT="$orfs_flow/platforms/sky130hd"
export WBQ_B4_PLACE_ODB="$place_odb"
export WBQ_B4_PLACE_SDC="$place_sdc"
export WBQ_B4_ROUTE_OUTPUT_ROOT="$artifacts"
export WBQ_B4_CUGR_CONGESTION_ITERATIONS=1
set +e
/usr/bin/time -v "$openroad_exe" -exit -no_init -threads 1 -no_splash "$tcl" >> "$log" 2>&1
route_rc=$?
set -e
end="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
echo "WBQ_B4_GLOBAL_ROUTE_EXIT_CODE=$route_rc" >> "$log"
echo "WBQ_B4_GLOBAL_ROUTE_END_UTC=$end" >> "$log"

analysis_rc=1
if [[ "$route_rc" -eq 0 ]] && grep -q "WBQ_B4_SINGLE_GLOBAL_ROUTE PASS" "$log" \
    && [[ -s "$guide" && -e "$congestion" && -s "$routed_odb" && -s "$routed_sdc" ]]; then
  set +e
  python3 tools/analyze_variant_residual_congestion.py \
    --variant B4 --pass-token "WBQ_B4_SINGLE_GLOBAL_ROUTE PASS" --cugr-iterations 1 \
...
payload = {
    ...
    "authorizes": ["B4_STRICT_PHASE6_EVALUATION"] if passed else [],
    "next_stage": "B4_STRICT_PHASE6_EVALUATION" if passed else None,
}
...
echo "B4 single global-route report: $manifest"
