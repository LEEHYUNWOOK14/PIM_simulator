...
physical="$root/reports/groot_normalization/quad_local_b7/physical"
...
odb="$artifacts/b7_place.odb"
sdc="$artifacts/b7_place.sdc"
tcl="$root/verification/groot_normalization/audit_wbq_b7_targeted_placement.tcl"
log="$physical/b7_targeted_placement_reopen_audit.log"
invocation="$physical/b7_targeted_placement_reopen_audit_invocation.json"
report="$physical/b7_targeted_placement_reopen_audit.json"
...
if placement.get('status') != 'PASS' or placement.get('placement_legality_and_fence_audit') != 'PASS':
    raise SystemExit('B7 placement manifest is not PASS')
for key,text in (('b7_place_odb',sys.argv[2]),('b7_place_sdc',sys.argv[3])):
    if sha(pathlib.Path(text)) != placement['outputs'][key]['sha256']:
        raise SystemExit(f'B7 targeted reopen input hash mismatch: {key}')
print('WBQ_B7_TARGETED_REOPEN_INPUT_GATE PASS')
PY
for path in "$log" "$invocation" "$report"; do
  if [[ -e "$path" ]]; then echo "existing B7 targeted reopen audit artifact prevents duplicate: $path" >&2; exit 4; fi
done
if pgrep -x openroad >/dev/null; then
  echo "OpenROAD already exists; refusing concurrent B7 targeted reopen audit" >&2
...
payload={
  'schema_version':1,'variant':'B7','stage':'targeted_placement_reopen_audit','status':'STARTED',
  ...
  'inputs':{
    ...
    'b7_place_odb':{'path':odb,'sha256':odb_sha},'b7_place_sdc':{'path':sdc,'sha256':sdc_sha},
...
PY
{
  echo "WBQ_B7_TARGETED_REOPEN_START_UTC=$start"
  echo "WBQ_B7_TARGETED_REOPEN_INVOCATION_COUNT=1"
  echo "WBQ_B7_TARGETED_REOPEN_ODB_SHA256=$odb_sha"
  echo "WBQ_B7_TARGETED_REOPEN_SDC_SHA256=$sdc_sha"
  echo "WBQ_B7_TARGETED_REOPEN_TCL_SHA256=$tcl_sha"
} > "$log"
termination_signal=""
compute_pid=""
on_signal() {
  termination_signal="$1"
  echo "WBQ_B7_TARGETED_REOPEN_SIGNAL=$termination_signal" >> "$log"
  if [[ -n "$compute_pid" ]] && kill -0 "$compute_pid" 2>/dev/null; then kill -TERM "$compute_pid"; fi
}
trap 'on_signal INT' INT
trap 'on_signal TERM' TERM
WBQ_B7_PLACE_ODB="$odb" WBQ_B7_PLACE_SDC="$sdc" \
...
echo "WBQ_B7_TARGETED_REOPEN_EXIT_CODE=$rc" >> "$log"
echo "WBQ_B7_TARGETED_REOPEN_END_UTC=$end" >> "$log"
...
anchors=re.findall(r'^WBQ_B7_TARGETED_AUDIT_ANCHOR name=\{(.+?)\} origin_dbu=\{(\d+) (\d+)\} orient=\{(\S+)\} status=\{(\S+)\}$',text,re.M)
fence=re.findall(r'^WBQ_B7_TARGETED_AUDIT_FENCES regions=(\d+) grouped=(\d+) outside=(\d+) unplaced=(\d+)$',text,re.M)
violations=re.findall(r'^WBQ_B7_TARGETED_AUDIT_LEGALITY violations=\{(.*)\}$',text,re.M)
passed=(int(rc)==0 and not signal and len(anchors)==2 and all(a[4]=='LOCKED' for a in anchors)
        ...
        and bool(violations) and violations[-1]=='' and 'WBQ_B7_TARGETED_PLACEMENT_REOPEN_AUDIT PASS' in text
...
payload={**inv,'status':'PASS' if passed else 'FAIL','end_utc':end,'exit_code':int(rc),'termination_signal':signal or None,
 ...
 'authorizes':['B7_GLOBAL_ROUTE_AUTHORIZATION'] if passed else [],
 'next_stage':'B7_GLOBAL_ROUTE_AUTHORIZATION' if passed else None}
