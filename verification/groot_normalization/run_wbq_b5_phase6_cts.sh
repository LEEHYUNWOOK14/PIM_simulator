...
artifact_root="${WBQ_B5_ARTIFACT_ROOT:-/dev/shm/wbq_b5_phase6_10/quad_local_b5}"
phase_root="${WBQ_B5_PHASE6_ROOT:-/dev/shm/wbq_b5_phase6_10/phase6_cts}"
work_home="${WBQ_B5_ORFS_WORK_HOME:-/dev/shm/wbq_b5_phase6_10/orfs}"
orfs_result="$work_home/results/sky130hd/normalization_hbm_quad_local_b5/base"
config="$root/flow/designs/sky130hd/normalization_hbm_quad_local_b5/config.mk"
gate="$b5_report/phase6_decision_gate.json"
placement="$b5_report/physical/b5_placement_execution_report.json"
place_odb="$artifact_root/b5_place.odb"
place_sdc="$artifact_root/b5_place.sdc"
cts_odb="$phase_root/b5_phase6_cts.odb"
cts_sdc="$phase_root/b5_phase6_cts.sdc"
log="$evidence/b5_phase6_cts.log"
audit_log="$evidence/b5_phase6_cts_audit.log"
preflight="$evidence/b5_phase6_cts_preflight.json"
attempt="$evidence/b5_phase6_cts_invocation.json"
manifest="$evidence/b5_phase6_cts_execution_report.json"
...
if gate.get("decision") != "PASS" or "B5_PHASE6_CTS" not in gate.get("authorizes", []):
    raise SystemExit("strict Phase 6 gate does not authorize B5 CTS")
if placement.get("status") != "PASS":
    raise SystemExit("B5 placement is not PASS")
for key, text in (("b5_place_odb", sys.argv[3]), ("b5_place_sdc", sys.argv[4])):
    path = pathlib.Path(text)
    if sha(path) != placement["outputs"][key]["sha256"]:
        raise SystemExit(f"B5 CTS input hash mismatch: {key}")
print("WBQ_B5_PHASE6_CTS_INPUT_GATE PASS")
PY
...
for path in "$preflight" "$attempt" "$manifest" "$log" "$audit_log" "$cts_odb" "$cts_sdc" \
  ...
  if [[ -e "$path" ]]; then
    echo "existing B5 Phase 6 CTS artifact prevents a fresh invocation: $path" >&2
    exit 4
  fi
done
...
payload = {
    "schema_version": 1, "phase": 6, "variant": "B5_PHASE6_CTS",
    "status": "PASS", "generated_at_utc": start,
    "command": "make -C ORFS WORK_HOME=<B5 tmpfs> ... do-4_1_cts",
    ...
    "authorizes": ["B5_PHASE6_CTS_COMPUTE"], "next_stage": "B5_PHASE6_CTS_COMPUTE",
}
...
{
  echo "WBQ_B5_PHASE6_CTS_START_UTC=$start"
  echo "WBQ_B5_PHASE6_CTS_INVOCATION_COUNT=1"
  echo "WBQ_B5_PHASE6_CTS_GATE_SHA256=$gate_sha"
  echo "WBQ_B5_PHASE6_CTS_PLACEMENT_SHA256=$placement_sha"
  echo "WBQ_B5_PHASE6_CTS_PLACE_ODB_SHA256=$place_odb_sha"
  echo "WBQ_B5_PHASE6_CTS_PLACE_SDC_SHA256=$place_sdc_sha"
  echo "WBQ_B5_PHASE6_CTS_POLICY=ORFS_TRITONCTS_REPAIR_CLOCK_NETS"
  echo "WBQ_B5_PHASE6_CTS_SIGNAL_LAYERS=met1-met5"
  echo "WBQ_B5_PHASE6_CTS_CLOCK_LAYERS=met2-met5"
} > "$log"
set +e
...
echo "WBQ_B5_PHASE6_CTS_EXIT_CODE=$rc" >> "$log"

audit_rc=1
if [[ "$rc" -eq 0 && -s "$cts_odb" && -s "$cts_sdc" ]]; then
  {
    echo "WBQ_B5_PHASE6_CTS_AUDIT_START_UTC=$(date -u +%Y-%m-%dT%H:%M:%SZ)"
    echo "WBQ_B5_PHASE6_CTS_AUDIT_ODB_SHA256=$(sha256sum "$cts_odb" | awk '{print $1}')"
    echo "WBQ_B5_PHASE6_CTS_AUDIT_SDC_SHA256=$(sha256sum "$cts_sdc" | awk '{print $1}')"
  } > "$audit_log"
  set +e
  WBQ_PLATFORM_ROOT="$orfs_flow/platforms/sky130hd" WBQ_CTS_ODB="$cts_odb" WBQ_CTS_SDC="$cts_sdc" \
    ...
    "$root/verification/groot_normalization/audit_wbq_b5_phase6_cts.tcl" \
    >> "$audit_log" 2>&1
  audit_rc=$?
  set -e
  echo "WBQ_B5_PHASE6_CTS_AUDIT_END_UTC=$(date -u +%Y-%m-%dT%H:%M:%SZ)" >> "$audit_log"
fi
end="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
echo "WBQ_B5_PHASE6_CTS_END_UTC=$end" >> "$log"
...
clock_nets = one(r"^WBQ_B5_PHASE6_CTS_AUDIT_CLOCK_NET_COUNT (\d+)$", audit_text)
passed = (int(rc) == 0 and int(audit_rc) == 0 and "WBQ_B5_PHASE6_CTS_AUDIT PASS" in audit_text
...
payload = {
    ...
    "authorizes": ["B5_PHASE6_POST_CTS_GLOBAL_ROUTE"] if passed else [],
    "next_stage": "B5_PHASE6_POST_CTS_GLOBAL_ROUTE" if passed else None,
...
echo "WBQ_B5_PHASE6_CTS PASS report=$manifest"
