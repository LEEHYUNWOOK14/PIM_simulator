module hierarchical_normalization_datapath_tb #(
 ...
 hierarchical_normalization_datapath #(.BANKS(B),.DATA_FORMAT(DATA_FORMAT)) dut(.clk_i(clk),.rst_ni(rst_n),
 ...
 task apply_one(input integer b,input logic[15:0]value,input logic last,input logic[15:0]expected);
  begin av[b]=1;atag[b]=tag;ax[b]=value;ag[b]=DATA_FORMAT?16'h3f80:16'h3c00;ab[b]=0;al[b]=last;
 ...
 initial begin
  bv=0;mode=0;tag=0;mask='1;counts={B{16'd4}};
  invh=DATA_FORMAT?16'h3d80:16'h2c00;eps=DATA_FORMAT?16'h0001:16'h0011;
  ...
  for(index=0;index<4;index=index+1)for(bank=0;bank<B;bank=bank+1)
    reduce_one(bank,DATA_FORMAT?16'h3f80:16'h3c00);
  wait(configured);if(mean!==0||inv!==(DATA_FORMAT?16'h3f7f:16'h3bfa))begin
  ...
  for(bank=0;bank<B;bank=bank+1)for(index=0;index<4;index=index+1)
    apply_one(bank,DATA_FORMAT?16'h3f80:16'h3c00,index==3,DATA_FORMAT?16'h3f7f:16'h3bfa);
  // LayerNorm: alternating -1/+1, mean=0 and variance=1.
  start(0,16'h202);
  for(index=0;index<4;index=index+1)for(bank=0;bank<B;bank=bank+1)
    reduce_one(bank,index[0]?(DATA_FORMAT?16'h3f80:16'h3c00):(DATA_FORMAT?16'hbf80:16'hbc00));
  wait(configured);if(mean!==0||inv!==(DATA_FORMAT?16'h3f7f:16'h3bfa))begin
    $display("HIER LN scalar mismatch mean=%h inv=%h",mean,inv);mismatches=mismatches+1;end
  for(bank=0;bank<B;bank=bank+1)for(index=0;index<4;index=index+1)
    apply_one(bank,index[0]?(DATA_FORMAT?16'h3f80:16'h3c00):(DATA_FORMAT?16'hbf80:16'hbc00),index==3,
      index[0]?(DATA_FORMAT?16'h3f7f:16'h3bfa):(DATA_FORMAT?16'hbf7f:16'hbbfa));
  ...
  $display("HIERARCHICAL_NORMALIZATION_DATAPATH_TB PASS format=%0d rows=2 banks=4 outputs=32",DATA_FORMAT);$finish;
 end
