...
"$py" tools/prepare_groot_mixed_precision_rtl_vectors.py
...
out="${TMPDIR:-/tmp}/groot_mixed_precision_trace.out";args=(-g2012 -Wall -I. -s groot_mixed_precision_trace_tb -o "$out");if [[ -n "$base" ]];then args=(-B "$base" "${args[@]}");fi
...
"$iv" "${args[@]}" "${src[@]}"
vectors=reports/groot_normalization/results/mixed_precision_rtl_vectors;results=reports/groot_normalization/results/mixed_precision_rtl_results;mkdir -p "$results";log="$results/rtl_runs.log";:>"$log"
run_case(){
  local p="$1" rows="$2" h="$3" v="$4" invh="$5" eps="$6"
  local cmd=("$vp")
  if [[ -n "$base" ]]; then cmd+=( -M "$base" ); fi
  cmd+=("$out" "+PROFILE=$p" "+ROWS=$rows" "+HIDDEN=$h" "+VECTORS=$v" "+INVH=$invh" "+EPS=$eps" "+MEANS=$vectors/${p}_mean_fp32.hex" "+INVS=$vectors/${p}_inv_std_fp32.hex" "+X=$vectors/${p}_x.hex" "+GAMMA=$vectors/${p}_gamma.hex" "+BETA=$vectors/${p}_beta.hex" "+MIXED=$vectors/${p}_mixed_expected.hex" "+PYTORCH=$vectors/${p}_pytorch_expected.hex" "+ACTUAL=$results/${p}_actual.hex")
  "${cmd[@]}" | tee -a "$log"
}
count=0
while IFS=, read -r p rows h v invh eps mean inv class; do
  p="${p//$'\r'/}"; rows="${rows//$'\r'/}"; h="${h//$'\r'/}"; v="${v//$'\r'/}"
  invh="${invh//$'\r'/}"; eps="${eps//$'\r'/}"
  [[ "$p" == profile_id ]] && continue
  [[ -n "${GROOT_PROFILE_FILTER:-}" && "$p" != "$GROOT_PROFILE_FILTER" ]] && continue
  run_case "$p" "$rows" "$h" "$v" "$invh" "$eps"
  count=$((count+1))
done < "$vectors/cases.csv"
echo "GROOT_MIXED_PRECISION_TRACE_REGRESSION PASS cases=$count"
