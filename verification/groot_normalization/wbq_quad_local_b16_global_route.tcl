foreach name {
  ...
  WBQ_B16_PLACE_ODB
  WBQ_B16_PLACE_SDC
  WBQ_B16_ROUTE_OUTPUT_ROOT
  WBQ_B16_CUGR_CONGESTION_ITERATIONS
} {
...
set iterations $::env(WBQ_B16_CUGR_CONGESTION_ITERATIONS)
if {![string is integer -strict $iterations] || $iterations != 1} {
  error "B16 congestion iterations must be exactly 1, got $iterations"
}
set platform_root $::env(WBQ_PLATFORM_ROOT)
set output_root $::env(WBQ_B16_ROUTE_OUTPUT_ROOT)
read_liberty $platform_root/lib/sky130_fd_sc_hd__tt_025C_1v80.lib
read_db $::env(WBQ_B16_PLACE_ODB)
read_sdc $::env(WBQ_B16_PLACE_SDC)
source $platform_root/setRC.tcl

# Keep B2's sealed I/O landing pattern and routing policy. B16 preserves the
...
puts "WBQ_B16_ROUTE_BUMP_PIN_GRID_COLUMNS $columns"
puts "WBQ_B16_ROUTE_BUMP_PIN_COUNT [llength $bump_terms]"
puts "WBQ_B16_ROUTE_CUGR_CONGESTION_ITERATIONS $iterations"
set_routing_layers -signal met1-met5 -clock met2-met5
global_route \
  -guide_file $output_root/b16_quad_local.route_guide \
  -congestion_report_file $output_root/b16_quad_local.congestion.rpt \
...
report_design_area
write_db $output_root/b16_quad_local_global_route.odb
write_sdc -no_timestamp $output_root/b16_quad_local_global_route.sdc
puts "WBQ_B16_SINGLE_GLOBAL_ROUTE PASS"
