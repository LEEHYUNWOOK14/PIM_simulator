...
for format in 0 1;do
 out="${TMPDIR:-/tmp}/normalization_special_values_f${format}.out"
 "$iv" -B "$base" -g2012 -s normalization_special_values_tb \
  -Pnormalization_special_values_tb.DATA_FORMAT="${format}" -o "$out" \
  ...
  verification/groot_normalization/normalization_special_values_tb.sv
 "$vvp" -M "$base" "$out"
done
