module groot_logic_die_pcu_trace_tb #(parameter int L=4, SCALAR_ENGINES=4, LOCAL_REDUCE_CONTEXTS=2, TOP_CONTEXTS=8, APPLY_FIFO_DEPTH=16);
  ...
  initial begin logic[15:0]request_tag;
    ...
    repeat(3)@(negedge clk);rst_n=1;@(negedge clk);invocation_v=1;@(posedge clk);if(!invocation_r)$fatal(1,"invocation descriptor rejected");@(negedge clk);invocation_v=0;
