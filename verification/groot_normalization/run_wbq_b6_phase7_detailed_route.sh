...
b6_report="$root/reports/groot_normalization/quad_local_b6"
phase6="$b6_report/phase6"
evidence="$b6_report/phase7"
input_root="${WBQ_B6_PHASE6_ROUTE_ROOT:-/dev/shm/wbq_b6_phase6_10/phase6_post_cts}"
output_root="${WBQ_B6_PHASE7_ROOT:-/dev/shm/wbq_b6_phase6_10/phase7}"
input_manifest="$phase6/b6_phase6_post_cts_global_route_execution_report.json"
authorization="$evidence/b6_phase7_detailed_route_authorization.json"
input_odb="$input_root/b6_phase6_post_cts_global_route.odb"
input_sdc="$input_root/b6_phase6_post_cts_global_route.sdc"
log="$evidence/b6_phase7_detailed_route.log"
audit_log="$evidence/b6_phase7_detailed_route_audit.log"
preflight="$evidence/b6_phase7_preflight.json"
attempt="$evidence/b6_phase7_detailed_route_invocation.json"
manifest="$evidence/b6_phase7_detailed_route_execution_report.json"
odb="$output_root/b6_phase7_detailed_route.odb"
sdc="$output_root/b6_phase7_detailed_route.sdc"
def="$output_root/b6_phase7_detailed_route.def"
netlist="$output_root/b6_phase7_detailed_route.v"
drc="$output_root/b6_phase7_detailed_route.drc.rpt"
antenna="$output_root/b6_phase7_antenna.rpt"
maze="$output_root/b6_phase7_detailed_route.maze.log"
timing="$output_root/b6_phase7_timing.rpt"
max_slew="$output_root/b6_phase7_max_slew.rpt"
max_capacitance="$output_root/b6_phase7_max_capacitance.rpt"
max_fanout="$output_root/b6_phase7_max_fanout.rpt"
tcl="$root/verification/groot_normalization/wbq_b6_phase7_detailed_route.tcl"
audit_tcl="$root/verification/groot_normalization/audit_wbq_b6_phase7_detailed_route.tcl"
runner="$root/verification/groot_normalization/run_wbq_b6_phase7_detailed_route.sh"
snapshot_tool="$root/tools/capture_openroad_stage_snapshot.py"
compare_tool="$root/tools/compare_openroad_stage_snapshots.py"
runner_service="${WBQ_RUNNER_SERVICE:-wbq-b6-phase7-detailed-route.service}"
...
if manifest.get("status") != "PASS" or "B6_PHASE7_DETAILED_ROUTE_AUTHORIZATION" not in manifest.get("authorizes", []):
    raise SystemExit("Phase 6 does not authorize B6 Phase 7 authorization")
if authorization.get("decision") != "PASS" or authorization.get("authorizes") != ["B6_PHASE7_DETAILED_ROUTE_COMPUTE"]:
    raise SystemExit("fresh B6 Phase 7 compute authorization is not PASS")
...
print(f"WBQ_B6_PHASE7_INPUT_GATE PASS available_ram_gib={mem_kib / 1024**2:.2f} free_disk_gib={free / 1024**3:.2f}")
PY
...
for path in "$preflight" "$attempt" "$manifest" "$log" "$audit_log" "$odb" "$sdc" "$def" \
  ...
  if [[ -e "$path" ]]; then echo "existing B6 Phase 7 artifact prevents overwrite: $path" >&2; exit 4; fi
done
...
payload = {"schema_version": 2, "phase": 7, "variant": "B6_PHASE7_DETAILED_ROUTE",
           ...
           "command": "openroad -threads 16 wbq_b6_phase7_detailed_route.tcl",
           ...
           "authorizes": ["B6_PHASE7_DETAILED_ROUTE_COMPUTE"],
           "next_stage": "B6_PHASE7_DETAILED_ROUTE_COMPUTE"}
...
on_signal() {
  ...
  echo "WBQ_B6_PHASE7_SIGNAL=$termination_signal" >> "$log"
...
payload = {
    "schema_version": 2, "phase": 7, "variant": "B6_PHASE7_DETAILED_ROUTE",
...
{
  echo "WBQ_B6_PHASE7_START_UTC=$start"
  echo "WBQ_B6_PHASE7_INVOCATION_COUNT=1"
  echo "WBQ_B6_PHASE7_SERVICE=$runner_service"
  echo "WBQ_B6_PHASE7_WRAPPER_PID=$$"
  echo "WBQ_B6_PHASE7_INPUT_MANIFEST_SHA256=$manifest_sha"
  echo "WBQ_B6_PHASE7_AUTHORIZATION_SHA256=$authorization_sha"
  echo "WBQ_B6_PHASE7_INPUT_ODB_SHA256=$odb_sha"
  echo "WBQ_B6_PHASE7_INPUT_SDC_SHA256=$sdc_sha"
  echo "WBQ_B6_PHASE7_SIGNAL_LAYERS=met1-met5"
  echo "WBQ_B6_PHASE7_CLOCK_LAYERS=met2-met5"
  echo "WBQ_B6_PHASE7_END_ITERATION=64"
} > "$log"
...
launcher_pid="$!"
echo "WBQ_B6_PHASE7_LAUNCHER_PID=$launcher_pid" >> "$log"
...
for _ in $(seq 1 120); do
  ...
  if [[ -n "$compute_pid" ]]; then
    echo "WBQ_B6_PHASE7_COMPUTE_PID=$compute_pid" >> "$log"
...
set -e
compute_pid=""
echo "WBQ_B6_PHASE7_EXIT_CODE=$route_rc" >> "$log"
echo "WBQ_B6_PHASE7_ROUTE_END_UTC=$(date -u +%Y-%m-%dT%H:%M:%SZ)" >> "$log"

audit_rc=1
if [[ "$route_rc" -eq 0 && -s "$odb" && -s "$sdc" ]]; then
  {
    echo "WBQ_B6_PHASE7_AUDIT_START_UTC=$(date -u +%Y-%m-%dT%H:%M:%SZ)"
    echo "WBQ_B6_PHASE7_AUDIT_ODB_SHA256=$(sha256sum "$odb" | awk '{print $1}')"
    echo "WBQ_B6_PHASE7_AUDIT_SDC_SHA256=$(sha256sum "$sdc" | awk '{print $1}')"
  } > "$audit_log"
...
PY
  ...
  set -e
  audit_compute_pid=""
  echo "WBQ_B6_PHASE7_AUDIT_END_UTC=$(date -u +%Y-%m-%dT%H:%M:%SZ)" >> "$audit_log"
fi
...
clock_nets = one(r"^WBQ_B6_PHASE7_AUDIT_CLOCK_NET_COUNT (\d+)$", audit)
signal_wires = one(r"^WBQ_B6_PHASE7_AUDIT_SIGNAL_WIRE_COUNT (\d+)$", audit)
...
passed = (int(route_rc) == 0 and int(audit_rc) == 0 and outputs_complete and drc_count is not None
          ...
          and "WBQ_B6_PHASE7_DETAILED_ROUTE_AUDIT PASS" in audit)
payload = {"schema_version": 2, "phase": 7, "variant": "B6_PHASE7_DETAILED_ROUTE",
           ...
           "authorizes": ["B6_PHASE7_RTL_GDS_STREAMOUT"] if passed else [],
           "next_stage": "B6_PHASE7_RTL_GDS_STREAMOUT" if passed else None,
...
echo "WBQ_B6_PHASE7_DETAILED_ROUTE PASS report=$manifest"
