...
smoke_odb="$artifacts/b8_smoke.odb"
smoke_tcl="$root/verification/groot_normalization/wbq_quad_local_b8_drc_smoke.tcl"
audit_tcl="$root/verification/groot_normalization/audit_wbq_b8_drc_smoke.tcl"
placement_tcl="$root/verification/groot_normalization/wbq_quad_local_b8_drc_place.tcl"
log="$physical/b8_smoke.log"
invocation="$physical/b8_smoke_invocation.json"
manifest="$physical/b8_smoke_execution_report.json"
...
for path in "$smoke_odb" "$log" "$invocation" "$manifest"; do
  if [[ -e "$path" ]]; then echo "existing B8 smoke artifact prevents duplicate: $path" >&2; exit 4; fi
done
if pgrep -x openroad >/dev/null; then echo "OpenROAD already exists; refusing concurrent B8 smoke" >&2; pgrep -a -x openroad >&2; exit 3; fi
free_kib="$(df -Pk "$artifacts" | awk 'NR==2 {print $4}')"
if (( free_kib < 6 * 1024 * 1024 )); then echo "less than 6 GiB free for B8 smoke checkpoint" >&2; exit 5; fi
...
if decision.get('decision')!='SELECT_B8_HIGHER_DRC_PENALTY_ECO' or decision.get('authorizes')!=[]:
    raise SystemExit('B8 recovery decision is not fail-closed')
...
if text.count('detailed_placement \\\n')!=1 or '-drc_penalty 100' not in text:
    raise SystemExit('B8 placement source lacks one-variable DRC policy')
if 'global_placement' in text or '-drc_penalty 20' in text or '-use_diamond_legalizer' in text:
    raise SystemExit('B8 placement source retains a failed or forbidden policy')
print('WBQ_B8_SMOKE_STATIC_POLICY PASS drc_penalty=100')
PY

sha() { sha256sum "$1" | awk '{print $1}'; }
start="$(date -u +%Y-%m-%dT%H:%M:%SZ)"; runner_service="${WBQ_RUNNER_SERVICE:-direct-cli-b8-smoke}"
...
p={'schema_version':1,'variant':'B8','stage':'b7_rudy_reopen_checkpoint_smoke','status':'STARTED','start_utc':start,
...
PY
{
  echo "WBQ_B8_SMOKE_START_UTC=$start"
  echo "WBQ_B8_SMOKE_INVOCATION_COUNT=1"
  echo "WBQ_B8_SMOKE_SERVICE=$runner_service"
  echo "WBQ_B8_SMOKE_WRAPPER_PID=$$"
  echo "WBQ_B8_SMOKE_STATIC_POLICY drc_penalty=100"
} > "$log"

compute_pid=""; audit_compute_pid=""; termination_signal=""
on_signal() {
  termination_signal="$1"; echo "WBQ_B8_SMOKE_SIGNAL=$termination_signal" >> "$log"
...
export WBQ_PLATFORM_ROOT="$orfs_flow/platforms/sky130hd" WBQ_B7_RUDY_ODB="$b7_rudy" WBQ_B2_PLACE_SDC="$b2_sdc" WBQ_B8_SMOKE_ODB="$smoke_odb"
...
end="$(date -u +%Y-%m-%dT%H:%M:%SZ)"; echo "WBQ_B8_SMOKE_EXIT_CODE=$rc" >> "$log"; echo "WBQ_B8_SMOKE_AUDIT_EXIT_CODE=$audit_rc" >> "$log"; echo "WBQ_B8_SMOKE_END_UTC=$end" >> "$log"
...
passed=(int(rc)==0 and int(audit_rc)==0 and not signal and bool(odb_sha) and preserved and p.get('compute_pid') and p.get('audit_compute_pid')
 and 'WBQ_B8_SMOKE_INPUT_REOPEN_AND_CHECKPOINT PASS anchors=2 offenders=7' in text
 and 'WBQ_B8_SMOKE_INDEPENDENT_REOPEN PASS anchors=2 offenders=7 fences=4' in text)
p.update({'status':'PASS' if passed else 'FAIL','end_utc':end,'exit_code':int(rc),'audit_exit_code':int(audit_rc),
 ...
 'checks':{'syntax_and_static_policy':'PASS','input_reopen':'PASS' if 'WBQ_B8_SMOKE_INPUT_REOPEN_AND_CHECKPOINT PASS' in text else 'FAIL',
 'target_objects':'PASS' if text.count('WBQ_B8_SMOKE_OFFENDER name=')==7 and text.count('WBQ_B8_SMOKE_ANCHOR name=')==2 else 'FAIL',
 'checkpoint_write':'PASS' if odb_sha else 'FAIL','independent_checkpoint_reopen':'PASS' if 'WBQ_B8_SMOKE_INDEPENDENT_REOPEN PASS' in text else 'FAIL'},
 'authorizes':['B8_PHYSICAL_AUTHORIZATION'] if passed else [],'next_stage':'B8_PHYSICAL_AUTHORIZATION' if passed else None})
...
echo "WBQ_B8_SMOKE PASS report=$manifest"
