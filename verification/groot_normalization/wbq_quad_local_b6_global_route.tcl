foreach name {
  ...
  WBQ_B6_PLACE_ODB
  WBQ_B6_PLACE_SDC
  WBQ_B6_ROUTE_OUTPUT_ROOT
  WBQ_B6_CUGR_CONGESTION_ITERATIONS
} {
...
set iterations $::env(WBQ_B6_CUGR_CONGESTION_ITERATIONS)
if {![string is integer -strict $iterations] || $iterations != 1} {
  error "B6 congestion iterations must be exactly 1, got $iterations"
}
set platform_root $::env(WBQ_PLATFORM_ROOT)
set output_root $::env(WBQ_B6_ROUTE_OUTPUT_ROOT)
...
# Keep B2's sealed I/O landing pattern and routing policy.  B6 changes only
# two explicitly locked legal anchors before the otherwise identical RUDY and
# negotiation placement flow.
set bump_terms {}
...
puts "WBQ_B6_ROUTE_BUMP_PIN_COUNT [llength $bump_terms]"
puts "WBQ_B6_ROUTE_CUGR_CONGESTION_ITERATIONS $iterations"
set_routing_layers -signal met1-met5 -clock met2-met5
global_route \
  -guide_file $output_root/b6_quad_local.route_guide \
  -congestion_report_file $output_root/b6_quad_local.congestion.rpt \
...
report_design_area
write_db $output_root/b6_quad_local_global_route.odb
write_sdc -no_timestamp $output_root/b6_quad_local_global_route.sdc
puts "WBQ_B6_SINGLE_GLOBAL_ROUTE PASS"
