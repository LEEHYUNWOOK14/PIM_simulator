...
evidence="$b5_report/phase6"
cts_root="${WBQ_B5_PHASE6_ROOT:-/dev/shm/wbq_b5_phase6_10/phase6_cts}"
route_root="${WBQ_B5_PHASE6_ROUTE_ROOT:-/dev/shm/wbq_b5_phase6_10/phase6_post_cts}"
cts_manifest="$evidence/b5_phase6_cts_execution_report.json"
cts_odb="$cts_root/b5_phase6_cts.odb"
cts_sdc="$cts_root/b5_phase6_cts.sdc"
log="$evidence/b5_phase6_post_cts_global_route.log"
audit_log="$evidence/b5_phase6_post_cts_route_audit.log"
preflight="$evidence/b5_phase6_post_cts_preflight.json"
attempt="$evidence/b5_phase6_post_cts_global_route_invocation.json"
manifest="$evidence/b5_phase6_post_cts_global_route_execution_report.json"
guide="$route_root/b5_phase6_post_cts.route_guide"
congestion="$route_root/b5_phase6_post_cts.congestion.rpt"
routed_odb="$route_root/b5_phase6_post_cts_global_route.odb"
routed_sdc="$route_root/b5_phase6_post_cts_global_route.sdc"
analysis_json="$evidence/b5_phase6_post_cts_congestion_analysis.json"
analysis_md="$evidence/b5_phase6_post_cts_congestion_analysis.md"
tcl="$root/verification/groot_normalization/wbq_b5_phase6_post_cts_global_route.tcl"
...
if manifest.get("status") != "PASS" or "B5_PHASE6_POST_CTS_GLOBAL_ROUTE" not in manifest.get("authorizes", []):
    raise SystemExit("B5 CTS gate does not authorize post-CTS global route")
...
payload = {
    "schema_version": 1, "phase": 6, "variant": "B5_PHASE6_POST_CTS",
    "status": "PASS", "generated_at_utc": start,
    "command": "openroad -threads 1 wbq_b5_phase6_post_cts_global_route.tcl",
    ...
    "authorizes": ["B5_PHASE6_POST_CTS_GLOBAL_ROUTE_COMPUTE"],
    "next_stage": "B5_PHASE6_POST_CTS_GLOBAL_ROUTE_COMPUTE",
}
...
{
  echo "WBQ_B5_PHASE6_POST_CTS_START_UTC=$start"
  echo "WBQ_B5_PHASE6_POST_CTS_GLOBAL_ROUTE_INVOCATION_COUNT=1"
  echo "WBQ_B5_PHASE6_POST_CTS_CUGR_ITERATIONS=10"
  echo "WBQ_B5_PHASE6_POST_CTS_SKIP_LARGE_FANOUT_NETS=20000"
  echo "WBQ_B5_PHASE6_POST_CTS_MANIFEST_SHA256=$cts_manifest_sha"
  echo "WBQ_B5_PHASE6_POST_CTS_ODB_SHA256=$cts_odb_sha"
  echo "WBQ_B5_PHASE6_POST_CTS_SDC_SHA256=$cts_sdc_sha"
} > "$log"
...
route_rc=$?
set -e
echo "WBQ_B5_PHASE6_POST_CTS_EXIT_CODE=$route_rc" >> "$log"
echo "WBQ_B5_PHASE6_POST_CTS_ROUTE_END_UTC=$(date -u +%Y-%m-%dT%H:%M:%SZ)" >> "$log"

audit_rc=1
if [[ "$route_rc" -eq 0 && -s "$routed_odb" && -s "$routed_sdc" ]]; then
  {
    echo "WBQ_B5_PHASE6_ROUTE_AUDIT_START_UTC=$(date -u +%Y-%m-%dT%H:%M:%SZ)"
    echo "WBQ_B5_PHASE6_ROUTE_AUDIT_ODB_SHA256=$(sha256sum "$routed_odb" | awk '{print $1}')"
    echo "WBQ_B5_PHASE6_ROUTE_AUDIT_SDC_SHA256=$(sha256sum "$routed_sdc" | awk '{print $1}')"
  } > "$audit_log"
  set +e
  WBQ_PLATFORM_ROOT="$orfs_flow/platforms/sky130hd" WBQ_ROUTED_ODB="$routed_odb" WBQ_ROUTED_SDC="$routed_sdc" \
    ...
    "$root/verification/groot_normalization/audit_wbq_b5_phase6_post_cts_route.tcl" \
    >> "$audit_log" 2>&1
  audit_rc=$?
  set -e
  echo "WBQ_B5_PHASE6_ROUTE_AUDIT_END_UTC=$(date -u +%Y-%m-%dT%H:%M:%SZ)" >> "$audit_log"
fi
...
if [[ "$route_rc" -eq 0 && "$audit_rc" -eq 0 && -s "$guide" && -e "$congestion" ]]; then
  ...
  python3 tools/analyze_variant_residual_congestion.py \
    --variant B5_PHASE6_POST_CTS \
    --pass-token 'WBQ_B5_PHASE6_POST_CTS_GLOBAL_ROUTE PASS' --cugr-iterations 10 \
...
payload = {
    ...
    "authorizes": ["B5_PHASE7_DETAILED_ROUTE"] if passed else [],
    "next_stage": "B5_PHASE7_DETAILED_ROUTE" if passed else None,
...
echo "WBQ_B5_PHASE6_POST_CTS_GLOBAL_ROUTE PASS report=$manifest"
