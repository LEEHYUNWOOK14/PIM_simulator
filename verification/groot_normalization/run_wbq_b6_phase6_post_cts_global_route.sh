...
cts_root="${WBQ_B6_PHASE6_ROOT:-/dev/shm/wbq_b6_phase6_10/phase6_cts}"
route_root="${WBQ_B6_PHASE6_ROUTE_ROOT:-/dev/shm/wbq_b6_phase6_10/phase6_post_cts}"
cts_manifest="$evidence/b6_phase6_cts_execution_report.json"
authorization="$evidence/b6_phase6_post_cts_route_authorization.json"
cts_odb="$cts_root/b6_phase6_cts.odb"
cts_sdc="$cts_root/b6_phase6_cts.sdc"
log="$evidence/b6_phase6_post_cts_global_route.log"
audit_log="$evidence/b6_phase6_post_cts_route_audit.log"
preflight="$evidence/b6_phase6_post_cts_preflight.json"
attempt="$evidence/b6_phase6_post_cts_global_route_invocation.json"
manifest="$evidence/b6_phase6_post_cts_global_route_execution_report.json"
guide="$route_root/b6_phase6_post_cts.route_guide"
congestion="$route_root/b6_phase6_post_cts.congestion.rpt"
routed_odb="$route_root/b6_phase6_post_cts_global_route.odb"
routed_sdc="$route_root/b6_phase6_post_cts_global_route.sdc"
analysis_json="$evidence/b6_phase6_post_cts_congestion_analysis.json"
analysis_md="$evidence/b6_phase6_post_cts_congestion_analysis.md"
...
runner_service="${WBQ_RUNNER_SERVICE:-wbq-b6-phase6-post-cts-route.service}"
...
if manifest.get("status") != "PASS" or "B6_PHASE6_POST_CTS_GLOBAL_ROUTE_AUTHORIZATION" not in manifest.get("authorizes", []):
    raise SystemExit("B6 CTS gate does not authorize post-CTS route authorization")
if authorization.get("decision") != "PASS" or "B6_PHASE6_POST_CTS_GLOBAL_ROUTE_COMPUTE" not in authorization.get("authorizes", []):
    raise SystemExit("fresh B6 post-CTS route authorization is not PASS")
for key, text in (("cts_odb", sys.argv[3]), ("cts_sdc", sys.argv[4])):
    path = pathlib.Path(text)
    if sha(path) != manifest["artifacts"][key]["sha256"]:
        raise SystemExit(f"B6 post-CTS route input hash mismatch: {key}")
for key, text in (
    ...
    if sha(path) != authorization["inputs"][key]["sha256"]:
        raise SystemExit(f"B6 post-CTS route authorization hash mismatch: {key}")
print("WBQ_B6_PHASE6_POST_CTS_INPUT_GATE PASS")
PY

for path in "$preflight" "$attempt" "$manifest" "$log" "$audit_log" "$guide" "$congestion" \
  "$routed_odb" "$routed_sdc" "$analysis_json" "$analysis_md"; do
  if [[ -e "$path" ]]; then
    echo "B6 post-CTS global route has already been attempted; refusing duplicate ($path)" >&2
    exit 4
  fi
done
if pgrep -x openroad >/dev/null; then
  echo "an OpenROAD process already exists; refusing concurrent B6 post-CTS route" >&2
...
if (( free_kib < 12 * 1024 * 1024 )); then
  echo "less than 12 GiB free for B6 post-CTS route" >&2
  exit 5
fi
if (( available_kib < 32 * 1024 * 1024 )); then
  echo "less than 32 GiB MemAvailable for B6 post-CTS route" >&2
  exit 6
fi
...
payload = {
    "schema_version": 2, "phase": 6, "variant": "B6_PHASE6_POST_CTS", "status": "STARTED",
    ...
    "command": "openroad -exit -no_init -threads 1 -no_splash wbq_b6_phase6_post_cts_global_route.tcl",
    ...
    "authorizes": ["B6_PHASE6_POST_CTS_GLOBAL_ROUTE_COMPUTE"],
    "next_stage": "B6_PHASE6_POST_CTS_GLOBAL_ROUTE_COMPUTE",
}
...
on_signal() {
  ...
  echo "WBQ_B6_PHASE6_POST_CTS_SIGNAL=$termination_signal" >> "$log"
  refresh_compute_pid
...
payload = {
    "schema_version": 2, "phase": 6, "variant": "B6_PHASE6_POST_CTS", "status": "FAIL",
...
{
  echo "WBQ_B6_PHASE6_POST_CTS_START_UTC=$start"
  echo "WBQ_B6_PHASE6_POST_CTS_GLOBAL_ROUTE_INVOCATION_COUNT=1"
  echo "WBQ_B6_PHASE6_POST_CTS_CUGR_ITERATIONS=10"
  echo "WBQ_B6_PHASE6_POST_CTS_SKIP_LARGE_FANOUT_NETS=20000"
  echo "WBQ_B6_PHASE6_POST_CTS_SERVICE=$runner_service"
  echo "WBQ_B6_PHASE6_POST_CTS_WRAPPER_PID=$$"
  echo "WBQ_B6_PHASE6_POST_CTS_MANIFEST_SHA256=$cts_manifest_sha"
  echo "WBQ_B6_PHASE6_POST_CTS_AUTHORIZATION_SHA256=$authorization_sha"
  echo "WBQ_B6_PHASE6_POST_CTS_ODB_SHA256=$cts_odb_sha"
  echo "WBQ_B6_PHASE6_POST_CTS_SDC_SHA256=$cts_sdc_sha"
} > "$log"
...
export WBQ_B6_PHASE6_CUGR_CONGESTION_ITERATIONS=10
/usr/bin/time -v "$openroad_exe" -exit -no_init -threads 1 -no_splash "$tcl" >> "$log" 2>&1 &
launcher_pid="$!"
echo "WBQ_B6_PHASE6_POST_CTS_LAUNCHER_PID=$launcher_pid" >> "$log"
...
for _ in $(seq 1 120); do
  ...
  if [[ -n "$compute_pid" ]]; then
    echo "WBQ_B6_PHASE6_POST_CTS_COMPUTE_PID=$compute_pid" >> "$log"
...
set -e
compute_pid=""
echo "WBQ_B6_PHASE6_POST_CTS_EXIT_CODE=$route_rc" >> "$log"
echo "WBQ_B6_PHASE6_POST_CTS_ROUTE_END_UTC=$(date -u +%Y-%m-%dT%H:%M:%SZ)" >> "$log"

audit_rc=1
if [[ "$route_rc" -eq 0 && -s "$routed_odb" && -s "$routed_sdc" ]]; then
  {
    echo "WBQ_B6_PHASE6_ROUTE_AUDIT_START_UTC=$(date -u +%Y-%m-%dT%H:%M:%SZ)"
    echo "WBQ_B6_PHASE6_ROUTE_AUDIT_ODB_SHA256=$(sha256sum "$routed_odb" | awk '{print $1}')"
    echo "WBQ_B6_PHASE6_ROUTE_AUDIT_SDC_SHA256=$(sha256sum "$routed_sdc" | awk '{print $1}')"
  } > "$audit_log"
...
PY
  ...
  set -e
  audit_compute_pid=""
  echo "WBQ_B6_PHASE6_ROUTE_AUDIT_END_UTC=$(date -u +%Y-%m-%dT%H:%M:%SZ)" >> "$audit_log"
fi
...
if [[ "$route_rc" -eq 0 && "$audit_rc" -eq 0 && -s "$guide" && -e "$congestion" ]]; then
  set +e
  python3 "$parser" \
    --variant B6_PHASE6_POST_CTS --pass-token 'WBQ_B6_PHASE6_POST_CTS_GLOBAL_ROUTE PASS' --cugr-iterations 10 \
...
passed = (int(route_rc) == 0 and int(audit_rc) == 0 and int(analysis_rc) == 0 and not signal
          ...
          and not skipped and preserved and "WBQ_B6_PHASE6_POST_CTS_ROUTE_AUDIT PASS" in pathlib.Path(audit_log).read_text(errors="replace"))
payload = {
    "schema_version": 2, "phase": 6, "variant": "B6_PHASE6_POST_CTS",
    ...
    "authorizes": ["B6_PHASE7_DETAILED_ROUTE_AUTHORIZATION"] if passed else [],
    "next_stage": "B6_PHASE7_DETAILED_ROUTE_AUTHORIZATION" if passed else None,
...
echo "WBQ_B6_PHASE6_POST_CTS_GLOBAL_ROUTE PASS report=$manifest"
