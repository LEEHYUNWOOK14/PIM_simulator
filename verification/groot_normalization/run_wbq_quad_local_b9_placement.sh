...
smoke="$physical/b9_smoke_execution_report.json"
authorization="$report/b9_physical_authorization.json"
...
post_legalize_odb="$artifacts/b9_post_legalize.odb"
b9_odb="$artifacts/b9_place.odb"
b9_sdc="$artifacts/b9_place.sdc"
log="$physical/b9_place.log"
attempt="$physical/b9_place_invocation.json"
manifest="$physical/b9_placement_execution_report.json"
tcl="$root/verification/groot_normalization/wbq_quad_local_b9_anchor_place.tcl"
...
for path in "$attempt" "$manifest" "$log" "$post_legalize_odb" "$b9_odb" "$b9_sdc"; do
  if [[ -e "$path" ]]; then echo "B9 placement has already been attempted: $path" >&2; exit 4; fi
done
if pgrep -x openroad >/dev/null; then echo "OpenROAD already exists; refusing concurrent B9 placement" >&2; pgrep -a -x openroad >&2; exit 3; fi
free_kib="$(df -Pk "$artifacts" | awk 'NR==2 {print $4}')"
available_kib="$(awk '/MemAvailable:/ {print $2}' /proc/meminfo)"
if (( free_kib < 15 * 1024 * 1024 )); then echo "less than 15 GiB free in B9 artifact storage" >&2; exit 5; fi
if (( available_kib < 40 * 1024 * 1024 )); then echo "less than 40 GiB MemAvailable for B9 placement" >&2; exit 6; fi
...
if decision.get('decision')!='SELECT_B9_SEVEN_ADDITIONAL_ANCHORS_ECO' or decision.get('authorizes')!=[]:
    raise SystemExit('B9 one-variable decision is not sealed')
if smoke.get('status')!='PASS' or smoke.get('authorizes')!=['B9_PHYSICAL_AUTHORIZATION']:
    raise SystemExit('B9 smoke is not PASS')
if auth.get('decision')!='PASS' or auth.get('authorizes')!=['B9_PLACEMENT']:
    raise SystemExit('B9 placement is not freshly authorized')
for name,item in auth.get('inputs',{}).items():
    path=pathlib.Path(item['path'])
    if not path.is_file() or sha(path)!=item['sha256']:
        raise SystemExit(f'B9 authorization input hash mismatch: {name}')
...
print('WBQ_B9_PLACEMENT_INPUT_GATE PASS anchor_targets=9 drc_penalty=100')
PY
...
p={'schema_version':1,'variant':'B9','stage':'nine_anchor_placement','status':'STARTED','start_utc':start,
 ...
 'policy':{'single_independent_change':'locked_anchor_targets','locked_anchor_targets_before':2,'locked_anchor_targets_after':9,
 'added_anchor_targets':7,'drc_penalty':100,'full_design_diamond':False,'sealed_b7_rudy_checkpoint':True,'post_legalize_checkpoint':True},
...
PY
{
  echo "WBQ_B9_PLACE_START_UTC=$start"
  echo "WBQ_B9_PLACE_INVOCATION_COUNT=1"
  echo "WBQ_B9_PLACE_GLOBAL_ROUTE_INVOCATIONS=0"
  echo "WBQ_B9_PLACE_SERVICE=$runner_service"
  echo "WBQ_B9_PLACE_WRAPPER_PID=$$"
  echo "WBQ_B9_PLACE_SINGLE_CHANGE=locked_anchor_targets"
  echo "WBQ_B9_PLACE_LOCKED_ANCHOR_TARGETS=9"
  echo "WBQ_B9_PLACE_DRC_PENALTY=100"
} > "$log"

launcher_pid=""; compute_pid=""; audit_compute_pid=""; termination_signal=""; finalized=0
on_signal() {
  termination_signal="$1"; echo "WBQ_B9_PLACE_SIGNAL=$termination_signal" >> "$log"
...
export WBQ_B9_POST_LEGALIZE_ODB="$post_legalize_odb" WBQ_B9_PLACE_ODB="$b9_odb" WBQ_B9_PLACE_SDC="$b9_sdc"
...
if [[ -z "$compute_pid" ]]; then echo "WBQ_B9_PLACE_COMPUTE_PID_DISCOVERY_FAIL" >> "$log"; kill -TERM "$launcher_pid" 2>/dev/null || true; fi
...
PY
echo "WBQ_B9_PLACE_LAUNCHER_PID=$launcher_pid" >> "$log"; echo "WBQ_B9_PLACE_COMPUTE_PID=${compute_pid:-NONE}" >> "$log"
set +e; wait "$launcher_pid"; rc=$?; set -e
echo "WBQ_B9_PLACE_EXIT_CODE=$rc" >> "$log"

audit_rc=1
if [[ "$rc" -eq 0 && -s "$b9_odb" && -s "$b9_sdc" ]]; then
  WBQ_QUAD_PLACE_ODB="$b9_odb" WBQ_QUAD_PLACE_SDC="$b9_sdc" \
...
echo "WBQ_B9_PLACE_AUDIT_EXIT_CODE=$audit_rc" >> "$log"
end="$(date -u +%Y-%m-%dT%H:%M:%SZ)"; echo "WBQ_B9_PLACE_END_UTC=$end" >> "$log"
item_sha() { [[ -s "$1" ]] && sha "$1" || true; }
post_sha="$(item_sha "$post_legalize_odb")"; odb_sha="$(item_sha "$b9_odb")"; sdc_sha="$(item_sha "$b9_sdc")"
...
python3 - "$manifest" "$attempt" "$log" "$end" "$rc" "$audit_rc" "$termination_signal" "$post_legalize_odb" "$post_sha" "$b9_odb" "$odb_sha" "$b9_sdc" "$sdc_sha" "$fa_after" "$fb_after" "$fb2_after" "$free_after_kib" "$mem_after_kib" <<'PY'
import hashlib,json,pathlib,sys
...
passed=(int(rc)==0 and int(audit_rc)==0 and not signal and bool(post_sha) and bool(odb_sha) and bool(sdc_sha) and preserved
 and p.get('compute_pid') and p.get('audit_compute_pid') and 'WBQ_B9_ANCHOR_RECOVERY_POLICY anchor_targets=9 added_targets=7 max_displacement={1000 1000} site_window=100 row_window=20 drc_penalty=100 full_design_diamond=0' in text
 and text.count('WBQ_B9_ANCHOR_VERIFIED name=')==9 and 'WBQ_B9_NINE_ANCHOR_PLACE PASS' in text
 and 'WBQ_QUAD_LOCAL_PLACEMENT_AUDIT PASS' in text and 'DPL-0033' not in text)
...
p.update({'status':'PASS' if passed else 'FAIL','end_utc':end,'exit_code':int(rc),'audit_exit_code':int(audit_rc),'termination_signal':signal or None,
 ...
 'outputs':{'b9_place_odb':{'path':odb,'sha256':odb_sha},'b9_place_sdc':{'path':sdc,'sha256':sdc_sha},
 'log':{'path':log,'sha256':hashlib.sha256(pathlib.Path(log).read_bytes()).hexdigest()}},
 'runtime_policy_marker':'PASS' if 'anchor_targets=9 added_targets=7' in text and 'drc_penalty=100 full_design_diamond=0' in text else 'FAIL',
...
echo "WBQ_B9_PLACEMENT PASS report=$manifest"
