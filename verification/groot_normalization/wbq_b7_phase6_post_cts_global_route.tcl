foreach name {
  ...
  WBQ_B7_PHASE6_CUGR_CONGESTION_ITERATIONS
} {
  if {![info exists ::env($name)] || $::env($name) eq ""} {
    error "missing required environment variable $name"
  }
}
set iterations $::env(WBQ_B7_PHASE6_CUGR_CONGESTION_ITERATIONS)
if {![string is integer -strict $iterations] || $iterations != 10} {
  error "B7 Phase 6 post-CTS congestion iterations must be 10"
}
...
puts "WBQ_B7_PHASE6_POST_CTS_BUMP_PIN_COUNT [llength $bump_terms]"
puts "WBQ_B7_PHASE6_POST_CTS_CUGR_ITERATIONS $iterations"
set_routing_layers -signal met1-met5 -clock met2-met5
global_route \
  -guide_file $output_root/b7_phase6_post_cts.route_guide \
  -congestion_report_file $output_root/b7_phase6_post_cts.congestion.rpt \
