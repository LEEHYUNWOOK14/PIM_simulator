#!/usr/bin/env bash
set -euo pipefail
repo_root="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
iv="${HOME}/.local/iverilog/usr/bin/iverilog"
vvp="${HOME}/.local/iverilog/usr/bin/vvp"
base="$(find "${HOME}/.local/iverilog/usr/lib" -type d -name ivl -print -quit)"
cd "${repo_root}"
python3 tools/generate_fp16_rsqrt_rtl_assets.py
python3 tools/generate_bf16_rsqrt_rtl_assets.py
python3 tools/generate_normalization_scalar_vectors.py --format FP16
python3 tools/generate_normalization_scalar_vectors.py --format BF16
for format in 0 1;do
  out="/tmp/logic_normalization_scalar_engine_f${format}_tb.out"
  "${iv}" -B "${base}" -g2012 -s logic_normalization_scalar_engine_tb \
    -Plogic_normalization_scalar_engine_tb.DATA_FORMAT="${format}" -o "${out}" \
    rtl/fp16_add.sv rtl/fp16_mul.sv rtl/bf16_add.sv rtl/bf16_mul.sv \
    rtl/fp16_rsqrt_lut256.sv rtl/bf16_rsqrt_lut256.sv \
    rtl/logic_normalization_scalar_engine.sv \
    verification/groot_normalization/logic_normalization_scalar_engine_tb.sv
  "${vvp}" -M "${base}" "${out}"
done
