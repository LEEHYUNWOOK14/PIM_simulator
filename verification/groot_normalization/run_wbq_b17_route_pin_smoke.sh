...
decision="$report/b17_eco_decision.json"
...
smoke_odb="$artifacts/b17_route_pin_smoke.odb"
smoke_tcl="$root/verification/groot_normalization/wbq_b17_route_pin_smoke.tcl"
audit_tcl="$root/verification/groot_normalization/audit_wbq_b17_route_pin_smoke.tcl"
log="$physical/b17_route_pin_smoke.log"
invocation="$physical/b17_route_pin_smoke_invocation.json"
manifest="$physical/b17_route_pin_smoke_execution_report.json"
...
for path in "$smoke_odb" "$log" "$invocation" "$manifest"; do
  [[ ! -e "$path" ]] || { echo "duplicate B17 smoke artifact: $path" >&2; exit 4; }
done
if pgrep -x openroad >/dev/null; then echo "OpenROAD already exists; refusing B17 smoke" >&2; exit 3; fi
python3 - "$decision" "$placement" "$smoke_tcl" "$audit_tcl" <<'PY'
import json, pathlib, sys
d=json.load(open(sys.argv[1])); p=json.load(open(sys.argv[2]))
if d.get("decision") != "SELECT_B17_BUMP_PITCH_ECO" or d.get("authorizes") != []:
    raise SystemExit("B17 decision not fail-closed")
...
if "set columns 25" not in pathlib.Path(sys.argv[3]).read_text() or "columns=25" not in pathlib.Path(sys.argv[4]).read_text():
    raise SystemExit("B17 grid marker missing")
print("WBQ_B17_SMOKE_STATIC_POLICY PASS columns=25")
PY
sha(){ sha256sum "$1" | awk '{print $1}'; }
start="$(date -u +%Y-%m-%dT%H:%M:%SZ)"
service="${WBQ_RUNNER_SERVICE:-direct-cli-b17-smoke}"
...
payload={"schema_version":1,"variant":"B17","stage":"route_pin_grid_smoke","status":"STARTED",
...
PY
{
  echo "WBQ_B17_SMOKE_START_UTC=$start"
  echo "WBQ_B17_SMOKE_INVOCATION_COUNT=1"
  echo "WBQ_B17_SMOKE_STATIC_POLICY columns=25"
} > "$log"
compute_pid=""
audit_compute_pid=""
termination_signal=""
on_signal(){
  ...
  echo "WBQ_B17_SMOKE_SIGNAL=$termination_signal" >> "$log"
...
WBQ_PLATFORM_ROOT="$orfs_flow/platforms/sky130hd" WBQ_B17_SMOKE_INPUT_ODB="$odb_in" \
WBQ_B17_SMOKE_INPUT_SDC="$sdc_in" WBQ_B17_SMOKE_OUTPUT_ODB="$smoke_odb" \
...
if [[ "$rc" -eq 0 && -s "$smoke_odb" ]]; then
  WBQ_B17_SMOKE_OUTPUT_ODB="$smoke_odb" "$openroad_exe" -exit -no_init -threads 1 -no_splash "$audit_tcl" >> "$log" 2>&1 &
  audit_compute_pid="$!"
...
echo "WBQ_B17_SMOKE_EXIT_CODE=$rc" >> "$log"
echo "WBQ_B17_SMOKE_AUDIT_EXIT_CODE=$audit_rc" >> "$log"
echo "WBQ_B17_SMOKE_END_UTC=$end" >> "$log"
smoke_sha=""
...
passed=(int(rc)==0 and int(arc)==0 and not signal and bool(odb_sha)
        and "WBQ_B17_SMOKE_INPUT_REOPEN_AND_CHECKPOINT PASS bump_terms=565 columns=25" in text
        and "WBQ_B17_SMOKE_INDEPENDENT_REOPEN PASS bump_terms=565 columns=25" in text)
data.update({"status":"PASS" if passed else "FAIL","end_utc":end,"exit_code":int(rc),
 ...
 "authorizes":["B17_ROUTE_AUTHORIZATION"] if passed else [],
 "next_stage":"B17_ROUTE_AUTHORIZATION" if passed else None})
...
echo "WBQ_B17_ROUTE_PIN_SMOKE PASS report=$manifest"
