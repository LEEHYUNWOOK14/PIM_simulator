...
mkdir -p "$report_dir"
netlist="$report_dir/${top}_sky130.v"
json="$report_dir/${prefix}_mapped_hierarchy.json"
log="$report_dir/${prefix}_sky130_mapping.log"
audit="$report_dir/${prefix}_mapped_locality_audit.json"
src=(
...
# Preserve the four ownership hierarchies through module-wise ABC.  A flat ABC
# run merges physically unrelated arithmetic cones and is both slower and less
# useful for the locality audit.
"$yosys_exe" -Q -l "$log" -p "
  read_liberty -lib -ignore_miss_func $sky130hd_liberty;
  read_verilog -sv -I. ${src[*]};
  hierarchy -check -top $top;
  synth -noabc -top $top;
  opt_clean;
  dfflibmap -liberty $sky130hd_liberty;
  abc -liberty $sky130hd_liberty -D 100000 \
  ...
  check -assert;
  stat -liberty $sky130hd_liberty -top $top;
  write_json $json;
...
python3 tools/audit_wbq_quad_local_netlist.py \
  --json "$json" --stage mapped --output "$audit" "${audit_args[@]}"
echo "NORMALIZATION_HBM_QUAD_LOCAL_AB_SKY130_MAPPING PASS netlist=$netlist"
