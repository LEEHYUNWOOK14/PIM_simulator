...
# The command/data boundary is internal to an HBM logic die.  Model its PHY or
# controller attachment as a distributed top-metal access array rather than as
# package pins forced onto the die perimeter.  Low-bandwidth job/status ports
# remain at the perimeter.
set bump_terms {}
...
foreach name $bump_terms {
  ...
  set x [expr {$x0 + $column * $pitch}]
  set y [expr {$y0 + $row * $pitch}]
  place_pin -pin_name $name -layer met5 -location [list $x $y] \
    -pin_size {20.0 20.0} -placed_status
  incr index
}
puts "V3_BUMP_PIN_COUNT [llength $bump_terms]"
puts "V3_BUMP_PIN_ARRAY x=${x0}..[expr {$x0 + ($columns - 1) * $pitch}] y=${y0}..[expr {$y0 + (([llength $bump_terms] - 1) / $columns) * $pitch}] pitch=$pitch"
puts "V3_BUMP_PIN_SMOKE_PASS"
