foreach name {WBQ_B7_PLACE_ODB WBQ_B7_PLACE_SDC} {
  if {![info exists ::env($name)] || $::env($name) eq ""} {
    error "missing required environment variable $name"
  }
}
read_db $::env(WBQ_B7_PLACE_ODB)
read_sdc $::env(WBQ_B7_PLACE_SDC)
set block [ord::get_db_block]
if {[$block getName] ne "logic_die_normalization_hbm_quad_local_b2_top"} {
  error "unexpected B7 placement top [$block getName]"
}
...
foreach spec $anchor_specs {
  ...
  if {$inst eq "NULL"} { error "B7 reopen audit anchor missing: $name" }
  lassign [$inst getOrigin] actual_x actual_y
  set actual_orient [$inst getOrient]
  set actual_status [$inst getPlacementStatus]
  puts "WBQ_B7_TARGETED_AUDIT_ANCHOR name={$name} origin_dbu={$actual_x $actual_y} orient={$actual_orient} status={$actual_status}"
  if {$actual_x != $expected_x || $actual_y != $expected_y ||
      $actual_orient ne $expected_orient || $actual_status ne "LOCKED"} {
    error "B7 reopen audit anchor mismatch: $name"
  }
  incr anchors_verified
}

set regions [$block getRegions]
if {[llength $regions] != 4} { error "expected four B7 quad fences" }
set grouped 0
set outside 0
foreach region $regions {
  if {[$region getRegionType] ne "EXCLUSIVE"} {
    error "B7 non-exclusive region [$region getName]"
  }
...
puts "WBQ_B7_TARGETED_AUDIT_FENCES regions=[llength $regions] grouped=$grouped outside=$outside unplaced=$unplaced"
puts "WBQ_B7_TARGETED_AUDIT_LEGALITY violations={$violations}"
puts "WBQ_B7_TARGETED_AUDIT_ANCHORS_VERIFIED $anchors_verified"
