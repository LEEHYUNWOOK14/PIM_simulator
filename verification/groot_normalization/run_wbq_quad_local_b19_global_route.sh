...
report="$root/reports/groot_normalization/quad_local_b19"
physical="$report/physical"
artifacts="${WBQ_B19_ARTIFACT_ROOT:-/dev/shm/wbq_b19_phase6_10/quad_local_b19}"
...
authorization="$report/b19_global_route_authorization.json"
...
log="$physical/b19_global_route.log"
guide="$artifacts/b19_quad_local.route_guide"
congestion="$artifacts/b19_quad_local.congestion.rpt"
routed_odb="$artifacts/b19_quad_local_global_route.odb"
routed_sdc="$artifacts/b19_quad_local_global_route.sdc"
attempt="$physical/b19_global_route_invocation.json"
manifest="$physical/b19_global_route_execution_report.json"
analysis_json="$report/b19_residual_congestion_analysis.json"
analysis_md="$report/b19_residual_congestion_analysis.md"
tcl="$root/verification/groot_normalization/wbq_quad_local_b19_global_route.tcl"
parser="$root/tools/analyze_variant_residual_congestion.py"
strict_gate="$root/tools/decide_b19_phase6_strict_gate.py"
snapshot_tool="$root/tools/capture_openroad_stage_snapshot.py"
compare_tool="$root/tools/compare_openroad_stage_snapshots.py"
runner="$root/verification/groot_normalization/run_wbq_quad_local_b19_global_route.sh"
runner_service="${WBQ_RUNNER_SERVICE:-wbq-b19-global-route.service}"
...
if placement.get("status") != "PASS" or placement.get("placement_legality_and_fence_audit") != "PASS":
    raise SystemExit("B19 placement and independent audit are not PASS")
...
if authorization.get("decision") != "PASS" or "B19_SINGLE_GLOBAL_ROUTE" not in authorization.get("authorizes", []):
    raise SystemExit("fresh B19 global-route authorization is not PASS")
...
for key, text in (
    ...
    if sha(path) != authorization["inputs"][key]["sha256"]:
        raise SystemExit(f"B19 route authorization hash mismatch: {key}")
targeted_path = pathlib.Path(sys.argv[11])
if sha(targeted_path) != authorization["inputs"]["targeted_placement_reopen_audit"]["sha256"]:
    raise SystemExit("B19 route authorization hash mismatch: targeted placement reopen audit")
numeric_path = pathlib.Path(sys.argv[12])
if sha(numeric_path) != authorization["inputs"]["placement_numeric_analysis"]["sha256"]:
    raise SystemExit("B19 route authorization hash mismatch: placement numeric analysis")
print("WBQ_B19_SINGLE_GLOBAL_ROUTE_AUTHORIZATION PASS")
PY

# Any one of these files proves that B19 consumed its sole route allowance.
for path in "$attempt" "$manifest" "$log" "$guide" "$congestion" "$routed_odb" "$routed_sdc" \
  "$analysis_json" "$analysis_md"; do
  if [[ -e "$path" ]]; then
    echo "B19 global route has already been attempted; refusing duplicate ($path)" >&2
    exit 4
  fi
done
if pgrep -x openroad >/dev/null; then
  echo "an OpenROAD process already exists; refusing concurrent B19 global route" >&2
...
if (( free_kib < 20 * 1024 * 1024 )); then
  echo "less than 20 GiB free in B19 artifact storage" >&2
  exit 5
fi
if (( available_kib < 32 * 1024 * 1024 )); then
  echo "less than 32 GiB MemAvailable for B19 global route" >&2
  exit 6
fi
...
payload = {
    ...
    "start_utc": start,
    "command": "openroad -exit -no_init -threads 1 -no_splash wbq_quad_local_b19_global_route.tcl",
    "service": service,
    ...
    "log": None,
    "expected_artifacts": [
        "b19_quad_local.route_guide", "b19_quad_local.congestion.rpt",
        "b19_quad_local_global_route.odb", "b19_quad_local_global_route.sdc",
    ],
    ...
    "artifact_storage": "B19-only tmpfs; compact hashes and reports persist in the repository",
}
...
on_signal() {
  ...
  echo "WBQ_B19_GLOBAL_ROUTE_SIGNAL=$termination_signal" >> "$log"
  refresh_compute_pid
...
{
  echo "WBQ_B19_GLOBAL_ROUTE_START_UTC=$start"
  echo "WBQ_B19_GLOBAL_ROUTE_INVOCATION_COUNT=1"
  echo "WBQ_B19_CUGR_CONGESTION_ITERATIONS=1"
  echo "WBQ_B19_GLOBAL_ROUTE_SERVICE=$runner_service"
  echo "WBQ_B19_GLOBAL_ROUTE_WRAPPER_PID=$$"
  echo "WBQ_B19_GLOBAL_ROUTE_PLACE_ODB_SHA256=$place_odb_sha"
  echo "WBQ_B19_GLOBAL_ROUTE_PLACE_SDC_SHA256=$place_sdc_sha"
} > "$log"
...
export WBQ_B19_PLACE_ODB="$place_odb"
export WBQ_B19_PLACE_SDC="$place_sdc"
export WBQ_B19_ROUTE_OUTPUT_ROOT="$artifacts"
export WBQ_B19_CUGR_CONGESTION_ITERATIONS=1
/usr/bin/time -v "$openroad_exe" -exit -no_init -threads 1 -no_splash "$tcl" >> "$log" 2>&1 &
launcher_pid="$!"
echo "WBQ_B19_GLOBAL_ROUTE_LAUNCHER_PID=$launcher_pid" >> "$log"
...
for _ in $(seq 1 120); do
  ...
  if [[ -n "$compute_pid" ]]; then
    echo "WBQ_B19_GLOBAL_ROUTE_COMPUTE_PID=$compute_pid" >> "$log"
...
echo "WBQ_B19_GLOBAL_ROUTE_EXIT_CODE=$route_rc" >> "$log"
echo "WBQ_B19_GLOBAL_ROUTE_END_UTC=$end" >> "$log"

analysis_rc=1
if [[ "$route_rc" -eq 0 ]] && grep -q "WBQ_B19_SINGLE_GLOBAL_ROUTE PASS" "$log" \
    && [[ -s "$guide" && -e "$congestion" && -s "$routed_odb" && -s "$routed_sdc" ]]; then
  set +e
  python3 "$parser" \
    --variant B19 --pass-token "WBQ_B19_SINGLE_GLOBAL_ROUTE PASS" --cugr-iterations 1 \
...
payload = {
    ...
    "authorizes": ["B19_STRICT_PHASE6_EVALUATION"] if passed else [],
    "next_stage": "B19_STRICT_PHASE6_EVALUATION" if passed else None,
}
with open(manifest, "x", encoding="utf-8") as stream:
    json.dump(payload, stream, indent=2)
    stream.write("\n")
...
echo "WBQ_B19_SINGLE_GLOBAL_ROUTE PASS report=$manifest"
