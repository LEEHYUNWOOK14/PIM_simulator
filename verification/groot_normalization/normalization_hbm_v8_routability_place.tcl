...
# Preserve V4's distributed physical boundary proxy.  Unlike V5, V8 creates
# no hard or soft hierarchy regions; congestion feedback alone may spread the
# context/writeback hotspot and any dense datapath neighborhoods.
set bump_terms {}
...
puts "V8_BUMP_PIN_COUNT [llength $bump_terms]"

puts "V8_PLACE_STAGE routability_global_placement"
global_placement \
  -skip_io \
  -density 0.42 \
  -routability_driven \
  ...
  -random_seed 42
write_db $report_root/logic_die_normalization_hbm_top_v8_routability_gp.odb

puts "V8_PLACE_STAGE detailed_placement"
detailed_placement \
  -max_displacement {500 500} \
  -report_file_name $report_root/logic_die_normalization_hbm_top_v8_placement_violations.rpt
check_placement -verbose \
  -report_file_name $report_root/logic_die_normalization_hbm_top_v8_check_placement.rpt
report_design_area
write_db $report_root/logic_die_normalization_hbm_top_v8_routability_legal.odb
write_sdc $report_root/logic_die_normalization_hbm_top_v8_routability_legal.sdc
puts "V8_ROUTABILITY_PLACE_PASS"
