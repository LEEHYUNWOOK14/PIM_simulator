foreach name {
  ...
  WBQ_B7_PLACE_ODB
  WBQ_B7_PLACE_SDC
  WBQ_B7_ROUTE_OUTPUT_ROOT
  WBQ_B7_CUGR_CONGESTION_ITERATIONS
} {
...
set iterations $::env(WBQ_B7_CUGR_CONGESTION_ITERATIONS)
if {![string is integer -strict $iterations] || $iterations != 1} {
  error "B7 congestion iterations must be exactly 1, got $iterations"
}
set platform_root $::env(WBQ_PLATFORM_ROOT)
set output_root $::env(WBQ_B7_ROUTE_OUTPUT_ROOT)
...
# Keep B2's sealed I/O landing pattern and routing policy.  B7 changes only
...
puts "WBQ_B7_ROUTE_BUMP_PIN_COUNT [llength $bump_terms]"
puts "WBQ_B7_ROUTE_CUGR_CONGESTION_ITERATIONS $iterations"
set_routing_layers -signal met1-met5 -clock met2-met5
global_route \
  -guide_file $output_root/b7_quad_local.route_guide \
  -congestion_report_file $output_root/b7_quad_local.congestion.rpt \
