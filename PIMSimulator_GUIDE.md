# PIMSimulator 저장소 안내서

## 1. 프로젝트 개요

`PIMSimulator`는 HBM2 기반 Processing-in-Memory(PIM) 시스템을 사이클 단위로 모델링하는 C++ 시뮬레이터다. 호스트가 발행하는 일반 메모리 트랜잭션과 DRAM 내부에서 수행되는 PIM 연산을 하나의 메모리 시스템 안에서 함께 시뮬레이션하며, 채널·랭크·뱅크·뱅크 상태·버스·명령 큐·전력 및 지연 통계를 모델링한다.
