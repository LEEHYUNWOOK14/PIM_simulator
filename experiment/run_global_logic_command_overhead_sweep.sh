...
BACKUP_DIR="$(mktemp -d)"
RESULT_FILE="$ROOT_DIR/experiment/results/global_logic_command_overhead_sweep.csv"
...
set_overhead() {
  ...
  for config_file in "${CONFIG_FILES[@]}"; do
    perl -0pi -e "s/^ENABLE_BANK_SIDE_PIM=.*/ENABLE_BANK_SIDE_PIM=true/m; \
                  ...
                  s/^LOGIC_CMD_OVERHEAD=.*/LOGIC_CMD_OVERHEAD=${overhead}/m; \
...
printf 'command_overhead,commands,dispatches,coalesced,dispatch_overhead_cycles,service_cycles,busy_until,cycle\n' \
  > "$RESULT_FILE"

for overhead in 0 1 2 4; do
  echo "[logic command overhead=$overhead cycles]"
