...
BACKUP_DIR="$(mktemp -d)"
RESULT_FILE="$ROOT_DIR/experiment/results/global_logic_unit_sweep.csv"
...
printf 'logic_units,groups,waves,commands,queue_cycles,service_cycles,busy_until,reads,writes,cycle\n' \
  > "$RESULT_FILE"

for units in 8 16 32 64; do
  echo "[global logic units=$units]"
  set_units "$units"
  ...
  printf '%s,%s,%s,%s,%s,%s,%s,%s,%s,%s\n' \
    "$units" "$(extract "$line" spatial_groups)" "$(extract "$line" batch_waves)" \
