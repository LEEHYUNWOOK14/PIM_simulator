...
for config_file in "${CONFIG_FILES[@]}"; do
  ...
  perl -0pi -e 's/^ENABLE_BANK_SIDE_PIM=.*/ENABLE_BANK_SIDE_PIM=true/m; \
                ...
                s/^LOGIC_PIM_LATENCY=.*/LOGIC_PIM_LATENCY=0/m; \
                s/^LOGIC_PIM_BW=.*/LOGIC_PIM_BW=0/m; \
                s/^HIERARCHY_PIM_BW=.*/HIERARCHY_PIM_BW=0/m; \
...
cd "$ROOT_DIR"
echo "[ideal hybrid] unlimited logic service and hierarchy bandwidth"
