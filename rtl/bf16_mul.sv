module bf16_mul (
    ...
    always @* begin
        ...
        if ((lhs_exp == 8'hff && lhs_frac != 0) ||
            ...
            result_o = {sign, 8'hff, 7'b0};
        else if ((lhs_exp == 0 && lhs_frac == 0) || (rhs_exp == 0 && rhs_frac == 0))
            result_o = {sign, 15'b0};
        else if (exponent >= 128) result_o = {sign, 8'hff, 7'b0};
            ...
            if (rounded_wide >= 256) begin
                rounded_wide = rounded_wide >> 1;
                exponent = exponent + 1;
            end
            if (exponent >= 128) result_o = {sign, 8'hff, 7'b0};
            else begin
                result_exp_field = exponent + 127;
                result_o = {sign, result_exp_field, rounded_wide[6:0]};
            end
        end else begin
            // A BF16 subnormal stores round(value / 2^-133).
            shift_amount = -(lhs_unbiased + rhs_unbiased + 119);
            if (shift_amount >= 64) rounded_wide = 0;
            ...
            if (rounded_wide >= 128) result_o = {sign, 8'd1, 7'd0};
            else result_o = {sign, 8'd0, rounded_wide[6:0]};
        end
    end
endmodule
