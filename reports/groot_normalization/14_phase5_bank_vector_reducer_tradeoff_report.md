...
현재 선택의 knee point는 **2 lanes/bank**다. Bank-only는 4 lanes에서 GPU-full 분석 가정을 근소하게 앞서지만, 1-port Hierarchical은 Logic partial network 병목 때문에 어떤 lane 수도 GPU-full보다 빠르지 않다. 8/16 lanes는 latency 개선 대비 bank별 복제 면적이 너무 크므로 권고하지 않는다.
...
|---:|---:|---:|---:|---|
| 1 | 107.395 ms | 1.00× | 617,929 | Logic-only |
| 2 | 89.196 ms | 1.20× | 786,617 | Logic-only |
| 4 | 80.097 ms | 1.34× | 1,123,561 | Logic-only |
| 8 | 76.981 ms | 1.40× | 1,797,353 | Logic-only |
| 16 | 75.423 ms | 1.42× | 3,144,937 | Logic-only |

참고 비교값은 Logic-only 16.397 ms, GPU full 36.773 ms다. 이 두 값에는 아직 실제 GPU 측정이 없고, Logic-only 면적에는 raw tensor reducer/apply가 빠져 있어 Logic-only 우승을 확정 결론으로 사용할 수 없다.

관찰:

- 2→16 lanes로 8배 확장해도 Hierarchical latency는 89.196→75.423 ms로 1.18배만 줄어든다. shared partial port와 global 단계가 지배하고, hidden=64 profile에서 8/16 lane이 활용되지 않기 때문이다.
- 1-port Hierarchical은 어느 lane 구성에서도 assumed GPU full보다 빠르지 않다.
- Bank-only도 4 lanes에서 35.612 ms로 assumed GPU full 36.773 ms를 근소하게 앞선다.
- 4-lane Hierarchical 면적 proxy는 1,123,561 cells다. 여기에는 reference의 16-engine shared dispatcher top 445,337 cells가 포함된다.

## 권고

- 면적을 중시하는 기본 후보: 2 lanes/bank
- Bank-only 성능 후보: 4 lanes/bank
